...
async def queue_merge(data_points: list = None, data_point_connections: list = None):
    queue.put((data_points, data_point_connections))
    return data_points, data_point_connections
...
async def consumer():
    while True:
        if queue.len != 0:
            ...
            nodes_and_edges = queue.get()

            await add_data_points(
                data_points=nodes_and_edges[0],
                data_point_connections=nodes_and_edges[1],
                index_edges=False,
            )
        else:

...
def _install_global_patch() -> None:
    ...
    async def patched_acompletion(*args, **kwargs):
        ...
        if _active_collector is not None:
            ...
            completion_tok = (getattr(usage, "completion_tokens", 0) or 0) if usage else 0
            if _active_collector._current is not None:
                _active_collector._current.llm_calls += 1
                _active_collector._current.prompt_tokens += prompt_tok
                _active_collector._current.completion_tokens += completion_tok
            else:
                # Parallel mode: accumulate bulk totals
                _active_collector._bulk_llm_calls += 1
                _active_collector._bulk_prompt_tokens += prompt_tok
                _active_collector._bulk_completion_tokens += completion_tok
        return response
...
class MetricsCollector:
    ...
    _bulk_llm_calls: int = field(default=0, repr=False)
    _bulk_prompt_tokens: int = field(default=0, repr=False)
    _bulk_completion_tokens: int = field(default=0, repr=False)

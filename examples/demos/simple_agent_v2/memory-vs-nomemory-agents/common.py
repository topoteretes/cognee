"""Shared logic for the streaming email offer demo."""
...
class NextToolDecision(BaseModel):
    """Controller output (same shape as job-finding agent NextToolDecision)."""
...
async def controller_decide_tool(
    *,
    ...
    """Pick the next tool via one structured LLM call (job-finding demo style)."""
    available_tools = list(ToolName)
    tool_list = ", ".join(t.value for t in available_tools)
    current_stage = (
        "PROPOSE"
        if not has_proposal
        else ("CHECK" if not has_check else "RETRY_OR_FINISH")
    )
    required_tool = {
    ...
    for email in load_emails():
        ...
        while loop_iteration < MAX_LOOP_ITERATIONS:
            ...
            if next_step.tool_name == ToolName.PROPOSE_OFFER:
                ...
                proposal = ProposalOutput.model_validate(proposal_payload)

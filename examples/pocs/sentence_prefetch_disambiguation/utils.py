...
def _normalize_nodes(nodes) -> set[tuple]:
    ...
    for node_id, props in nodes:
        ...
        if ntype == "TextDocument" or ntype == "TextSummary" or ntype == "DocumentChunk":
            continue
...
def _diff_message(name_a: str, set_a: set, name_b: str, set_b: set) -> str:
    ...
    return (
        ...
        f"Only in {name_a}: {only_a[:10]}{'...' if len(only_a) > 50 else ''}. "
        f"Only in {name_b}: {only_b[:10]}{'...' if len(only_b) > 50 else ''}."
    )
...
def _compare(name_a: str, set_a: set, name_b: str, set_b: set) -> None:
    ...
    only_b = set_b - set_a
    common = set_a & set_b
    print(f" {len(set_a)} in {name_a} vs {len(set_b)} in {name_b}: {len(common)} common")
    if only_a:
        print(
            f"    Only in {name_a}: {sorted(only_a, key=lambda t: t[1])[:10]}{'...' if len(only_a) > 10 else ''}"
        )
    if only_b:
        print(
            f"    Only in {name_b}: {sorted(only_b, key=lambda t: t[1])[:10]}{'...' if len(only_b) > 10 else ''}"
        )

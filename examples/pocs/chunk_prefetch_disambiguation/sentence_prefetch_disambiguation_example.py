...
async def main(
    example,
    use_sentence_prefetch_disambiguation: Optional[bool] = False,
    ...
    graph_visualization_path = os.path.join(
        ...
        f"results/{'poc_' if use_sentence_prefetch_disambiguation else ''}cognify_disambiguate_{example}_result.html",
    )

    parts_dir = Path(__file__).resolve().parent / "data" / example
    kwargs = {}
    if use_sentence_prefetch_disambiguation:
...
async def _run():
    ...
    await main(
        example="example1",
        use_sentence_prefetch_disambiguation=True,

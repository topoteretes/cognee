...
(graph-as-tables) and the vector store (exact cosine search), each a local database
file under the chosen system directory.
...
providers are forced to ``turso`` by this script (overriding ``.env``), so no ``.env``
...
for variable in ("DB_PROVIDER", "GRAPH_DATABASE_PROVIDER", "VECTOR_DB_PROVIDER"):

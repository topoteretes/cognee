...
async def test_add_subgraph_to_layer(layered_graph_fixture):
    ...
    builder = LayeredGraphBuilder(
        name=layered_graph.name,
        description=layered_graph.description
    )
...
async def test_layered_graph_service_diff_layers(layered_graph_fixture):
    ...
    base_edge_tuples = {(edge.source_node_id, edge.target_node_id, edge.relationship_name) for edge in base_edges}
    second_edge_tuples = {(edge.source_node_id, edge.target_node_id, edge.relationship_name) for edge in second_edges}
...
async def test_layered_graph_service_merge_layers(layered_graph_fixture):
    ...
    builder = LayeredGraphBuilder(
        name="Merged Graph",
        description="A graph with merged layers"
    )
    
    # Create a merged layer
    merged_layer_id = builder.create_layer(
        name="Merged Layer",
        description="Merged from base and second layers",
        layer_type="merged"
    )
...
async def test_layered_graph_service_extract_subgraph(layered_graph_fixture):
    ...
    filtered_edges = [
        edge for edge in base_edges + second_edges
...
async def test_layered_graph_service_calculate_metrics(layered_graph_fixture):
    ...
    for layer in layers:
        ...
        cumulative_nodes, cumulative_edges = await layered_graph.get_cumulative_layer_graph(layer_id)
...
async def test_convert_regular_graph_to_layered():
    ...
    builder = LayeredGraphBuilder(
        ...
        description="Graph converted from regular to layered"
    )
    
    # Create a base layer
    base_layer_id = builder.create_layer(
        name="Base Layer",
        description="Base layer from conversion",

...
if not hasattr(LayeredKnowledgeGraphDP, "get_cumulative_layer_graph"):
    ...
    def get_cumulative_layer_graph(self, layer_id):
        ...
        ]  # Fixed: was using 'edges' instead of 'all_edges'
...
class TestDataPointLayeredGraphBasic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph = LayeredKnowledgeGraphDP.create_empty(
            name="Test Graph", description="Test graph for unit tests"
        )
        ...
        self.company_node = GraphNode(
            id=uuid4(), name="ACME Corp", node_type="Company", description="A fictional company"
        )
    ...
    def test_layer_management(self):
        ...
        parent_layers = (
            self.graph.get_parent_layers(self.detail_layer.id)
            if hasattr(self.graph, "get_parent_layers")
            else [
                self.graph.layers[pid]
                for pid in self.graph.layers[self.detail_layer.id].parent_layers
            ]
        )
...
class TestAsyncFunctionality(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.graph = LayeredKnowledgeGraphDP.create_empty(
            name="Async Test Graph", description="Graph for testing async operations"
        )
    ...
    async def test_async_node_addition(self):
        ...
        for i in range(10):
            node = GraphNode(
                id=uuid4(), name=f"Node {i}", node_type="TestNode", description=f"Test node {i}"
            )
            nodes.append(node)
        ...
        for node in nodes[:5]:
            tasks.append(
                asyncio.create_task(self.graph.add_node_to_layer_async(node, self.base_layer.id))
            )
    ...
    async def test_async_edge_addition(self):
        ...
        node_a = GraphNode(
            id=uuid4(), name="Node A", node_type="TestNode", description="Test node A"
        )
        node_b = GraphNode(
            id=uuid4(), name="Node B", node_type="TestNode", description="Test node B"
        )
        node_c = GraphNode(
            id=uuid4(), name="Node C", node_type="TestNode", description="Test node C"
        )
        ...
        tasks = [
            ...
            asyncio.create_task(
                self.graph.add_edge_to_layer_async(edge_bc, self.enrichment_layer.id)
            ),
            asyncio.create_task(
                self.graph.add_edge_to_layer_async(edge_ca, self.enrichment_layer.id)
            ),
        ]
    ...
    async def test_async_graph_build(self):
        ...
        for i in range(len(base_nodes) - 1):
            ...
            edge_tasks.append(
                asyncio.create_task(self.graph.add_edge_to_layer_async(edge, self.base_layer.id))
            )
        ...
        self.assertEqual(
            len(self.graph.get_layer_edges(self.base_layer.id)), 4
        )  # 5 nodes - 1 = 4 edges
        self.assertEqual(
            len(self.graph.get_layer_edges(self.enrichment_layer.id)), 3
        )  # 3 enrichment nodes

...
async def test_edge_ingestion():
    ...
    data_directory_path = str(
        pathlib.Path(
            os.path.join(
                pathlib.Path(__file__).parent, ".data_storage/test_edge_ingestion"
            )
        ).resolve()
    )
    cognee_directory_path = str(
        pathlib.Path(
            os.path.join(
                pathlib.Path(__file__).parent, ".cognee_system/test_edge_ingestion"
            )
        ).resolve()
    )
    ...
    for basic_nested_edge in basic_nested_edges:
        assert (
            edge_type_counts.get(basic_nested_edge, 0) >= 1
    ...
    assert edge_type_counts.get("made_from", 0) == edge_type_counts.get(
        "is_part_of", 0
    ...
    assert edge_type_counts.get("contains", 0) == edge_type_counts.get(
        "is_a", 0

...
async def test_service_management(client: MCPClient):
    ...
    try:
        ...
        if service_details.get("can_restart", False) and not service_details.get("critical", True):
            ...
            print(
                f"Service {service_id} does not support restart or is critical. Skipping restart test."
            )
...
def main():
    ...
    parser.add_argument(
        "--api-key", default=os.environ.get("COGNEE_MCP_API_KEY"), help="API key for authentication"
    )
    parser.add_argument(
        "--test",
        choices=[
    ...
    # Create MCP client
    client = MCPClient(base_url=args.url, api_key=args.api_key, timeout=args.timeout)

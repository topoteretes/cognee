...
class DockerImageTester:
    ...
    def pull_image(self) -> bool:
        ...
        exit_code, stdout, stderr = self.run_command(["docker", "pull", self.full_image])
    ...
    def get_container_exit_info(self, container_id: str) -> Dict[str, Any]:
        ...
        exit_code, stdout, stderr = self.run_command(["docker", "inspect", container_id])
    ...
    def stop_container(self, container_id: str) -> bool:
        ...
        exit_code, stdout, stderr = self.run_command(["docker", "stop", container_id])
    ...
    def remove_container(self, container_id: str) -> bool:
        ...
        exit_code, stdout, stderr = self.run_command(["docker", "rm", container_id])
    ...
    def check_container_logs(self, container_id: str, follow: bool = False) -> str:
        ...
        if not stdout.strip():
            ...
            if len(log_lines) > 10:
                self.log(
                    f"Container logs (first 10 lines):\n" + "\n".join(log_lines[:10]) + "\n[...]"
                )
            else:
                self.log(f"Container logs:\n{stdout}")
    ...
    def test_container_functionality(self, container_id: str, retry_count: int = 5) -> bool:
        ...
        for i in range(retry_count):
            ...
            try:
                ...
                self.log(
                    f"Attempt {i + 1}/{retry_count}: Failed to connect to server in container: {e}"
                )
    ...
    def run_full_test(self) -> bool:
        ...
        container_id = self.run_container(container_name=container_name, env_vars=default_env_vars)
        ...
        try:
            ...
            if not self.check_container_status(container_id):
                ...
                if self.debug_mode:
                    self.run_container_with_command(
                        ["/bin/sh", "-c", "ls -la / && env && echo 'Testing container startup...'"]
                    )
...
def main():
    ...
    parser.add_argument(
        "--image-name", default="cognee/cognee-mcp", help="Name of the Docker image to test"
    )
    parser.add_argument(
        "--image-tag", default="main", help="Tag of the Docker image to test (default: main)"
    )
    parser.add_argument("--port", default="8080", help="Port to use for testing")
    parser.add_argument(
        "--mcp-url",
        default=os.environ.get("COGNEE_MCP_URL", "http://localhost:8080"),
        help="URL of an existing MCP server (if available)",
    )
    parser.add_argument(
        "--api-key", default=os.environ.get("COGNEE_MCP_API_KEY"), help="API key for the MCP server"
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument(
        "--debug", action="store_true", help="Run container in debug mode (interactive)"
    )
    parser.add_argument(
        "--env", action="append", help="Set environment variables in the format KEY=VALUE"
    )
    ...
    # Create MCP client
    client = MCPClient(base_url=args.mcp_url, api_key=args.api_key, debug=args.verbose)

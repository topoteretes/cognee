...
async def test_improve_permission_matrix(permissions_example_env):
    ...
    6. write grant + session bound elsewhere -> SessionDatasetMismatchError
    ...
    from cognee.modules.session_lifecycle import SessionDatasetMismatchError
    ...
    # 6. Write permission does not override the session's dataset binding.
    session_id = "improve-perms-session"
    await record_session_activity(str(other.id), session_id, dataset_id=str(dataset_2_id))
    with pytest.raises(SessionDatasetMismatchError):
        await cognee.improve(dataset=dataset_id, session_ids=[session_id], user=other)

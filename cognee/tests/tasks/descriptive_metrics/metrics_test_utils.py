...
async def create_disconnected_test_graph():
    ...
    doc_chunk2 = DocumentChunk(
        part_of=doc2, text="This is a chunk of text", contains=[]
    )
...
async def assert_metrics(provider, include_optional=True):
    ...
    diff_keys = set(metrics.keys()).symmetric_difference(
        set(ground_truth_metrics.keys())
    )
    if diff_keys:

...
async def create_test_graph() -> LayeredKnowledgeGraphDP:
    ...
    graph = LayeredKnowledgeGraphDP.create_empty(
        name="Test Layered Graph", description="A test layered graph for verification"
    )
    ...
    executive_node = GraphNode.create(
        name="Executive", node_type="Category", description="Company executives", properties={}
    )
...
async def test_store_and_retrieve_graph(
    adapter: LayeredGraphDBAdapter, test_graph: LayeredKnowledgeGraphDP
):
    """
...
async def test_layer_operations(
    adapter: LayeredGraphDBAdapter, graph_id: Union[str, UUID], graph: LayeredKnowledgeGraphDP
):
    """
    ...
    try:
        ...
        if merged_layer:
            ...
            merged_nodes = [
                node for node in updated_graph.nodes.values() if node.layer_id == merged_layer.id
            ]
            merged_edges = [
                edge for edge in updated_graph.edges.values() if edge.layer_id == merged_layer.id
            ]

            logger.info(
                f"Merged layer contains {len(merged_nodes)} nodes and {len(merged_edges)} edges"
            )
        else:
...
async def test_cross_layer_relationships(
    adapter: LayeredGraphDBAdapter, graph_id: Union[str, UUID]
):
    """
    ...
    try:
        ...
        for rel in relationships:
            logger.info(
                f"Relationship: {rel['source_layer']} -> {rel['target_layer']} ({rel['relationship_type']})"
            )

...
| `test_*.py` (top level, ~70 files) | Scenario tests, each run by its own CI job: delete semantics (`test_delete_*`), permissions, search DB matrix (`test_search_db.py`), custom models, code graph, usage logging, … | most need `LLM_API_KEY`; the backend ones need that backend |
...
```bash
uv run pytest                                  # = cognee/tests (testpaths in pyproject)

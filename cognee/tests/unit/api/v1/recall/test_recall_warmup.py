...
async def test_build_failed_returns_diagnostic_marker(
    ...
    assert marker.error_category == "auth"
    assert "AuthenticationError: invalid api key" in marker.text
    assert stubbed_graph_lane == []


@pytest.mark.asyncio
async def test_build_in_progress_returns_marker(
    monkeypatch,
    api_recall_mod,
    graph_warmup_mod,
    recall_config_mod,
    ...
    _stub_probe(
        monkeypatch,
        graph_warmup_mod,
        graph_warmup_mod.WarmupProbe(graph_warmup_mod.STATE_BUILD_IN_PROGRESS, 0),
    )
    graph_warmup_mod.clear_warmup_cache()
    ...
    assert len(out) == 1
    assert out[0].status == "build_in_progress"
    assert out[0].error_class is None
    assert stubbed_graph_lane == []

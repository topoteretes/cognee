...
def test_adapter_can_instantiate_and_load(AdapterClass):
    ...
    assert isinstance(
        corpus_list, list
    ), f"{AdapterClass.__name__} corpus_list is not a list."
    assert isinstance(
        qa_pairs, list
...
def test_adapter_returns_some_content(AdapterClass):
    ...
    assert (
        len(corpus_list) > 0
    ), f"{AdapterClass.__name__} returned an empty corpus_list."
    assert (
        len(qa_pairs) > 0
    ), f"{AdapterClass.__name__} returned an empty question_answer_pairs."
    assert (
        len(qa_pairs) <= limit
    ...
    for item in qa_pairs:
        assert (
            "question" in item
        ), f"{AdapterClass.__name__} missing 'question' key in QA pair."
        assert (
            "answer" in item
        ), f"{AdapterClass.__name__} missing 'answer' key in QA pair."

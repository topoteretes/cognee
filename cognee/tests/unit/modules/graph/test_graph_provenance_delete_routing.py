...
async def test_delete_data_routes_graph_provenance():
    ...
    with (
        ...
        patch.object(ddne_module, "_invalidate_bm25_cache") as invalidate_cache,
    ):
    ...
    invalidate_cache.assert_called_once_with(dataset_id)
...
async def test_delete_dataset_routes_graph_provenance():
    ...
    with (
        ...
        patch.object(ddsne_module, "_invalidate_bm25_cache") as invalidate_cache,
    ):
...
async def test_delete_dataset_old_graph_invalidates_after_legacy_cleanup():
    dataset_id, user_id = uuid4(), uuid4()
    unified = _unified(graph_provenance_supported=False)
    ...
    with (
        ...
        patch.object(ddsne_module, "get_unified_engine", AsyncMock(return_value=unified)),
        patch.object(ddsne_module, "backend_access_control_enabled", lambda: False),
        patch.object(ddsne_module, "get_global_dataset_related_nodes", AsyncMock(return_value=[])),
        patch.object(ddsne_module, "delete_dataset_related_nodes", AsyncMock()) as delete_nodes,
        patch.object(ddsne_module, "delete_dataset_related_edges", AsyncMock()) as delete_edges,
        patch.object(ddsne_module, "_invalidate_bm25_cache") as invalidate_cache,
    ):
        await ddsne_module.delete_dataset_nodes_and_edges(dataset_id, user_id)

    delete_nodes.assert_awaited_once_with(dataset_id)
    delete_edges.assert_awaited_once_with(dataset_id)
    invalidate_cache.assert_called_once_with(dataset_id)

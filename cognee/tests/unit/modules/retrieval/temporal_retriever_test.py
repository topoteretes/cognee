...
async def test_extract_time_from_query_now_is_timezone_aware():
    ...
    # Parsed as an explicitly UTC instant: the rendered string *is* a UTC
    # calendar date, and leaving it naive both trips the repository's DTZ
    # rule and re-introduces the local-vs-UTC ambiguity this test pins.
    parsed = datetime.strptime(captured["time_now"], "%d-%m-%Y").replace(tzinfo=timezone.utc).date()

...
@pytest.mark.asyncio
async def test_get_users_in_role_scopes_role_to_authorized_tenant(monkeypatch):
    tenant_id = uuid4()
    role_id = uuid4()
    requester_id = uuid4()
    target_user_id = uuid4()
    target_user = SimpleNamespace(id=target_user_id, email="member@example.com")
    session = FakeSession([SimpleNamespace(id=role_id), None, [target_user]])
    permission_calls = []
    ...
    assert "user_roles.role_id" in statement_sql(session, 2)
...
@pytest.mark.asyncio
async def test_get_user_roles_checks_permission_before_looking_up_target(monkeypatch):
    requester_id, tenant_id, target_user_id = uuid4(), uuid4(), uuid4()
    permission_check = AsyncMock(side_effect=PermissionDeniedError())
    engine_factory = Mock()
    monkeypatch.setattr(_get_user_roles_mod, "get_relational_engine", engine_factory)
    monkeypatch.setattr(_get_user_roles_mod, "has_user_management_permission", permission_check)

    with pytest.raises(PermissionDeniedError):
        await get_user_roles(tenant_id, target_user_id, SimpleNamespace(id=requester_id))

    permission_check.assert_awaited_once_with(requester_id, tenant_id)

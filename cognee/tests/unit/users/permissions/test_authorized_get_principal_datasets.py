...
def _patch(monkeypatch, principal, datasets=(), role_names=(), may_manage=False):
    """Wire the method's collaborators. has_user_management_permission mirrors
    the real one: True when allowed, PermissionDeniedError when not."""
...
async def test_user_may_read_their_own_datasets(monkeypatch):
    ...
    datasets = await authorized_get_principal_datasets(
        requester_id, "read", requester_id, tenant_id
    )
...
async def test_another_users_datasets_need_user_management(monkeypatch):
    ...
    _patch(monkeypatch, other, datasets=[FakeDataset(tenant_id)], may_manage=False)

    with pytest.raises(PermissionDeniedError):
        await authorized_get_principal_datasets(other.id, "read", requester_id, tenant_id)
...
async def test_user_manager_may_read_another_users_datasets(monkeypatch):
    requester_id, tenant_id = uuid4(), uuid4()
    other = FakePrincipal("user")
    _patch(monkeypatch, other, datasets=[FakeDataset(tenant_id)], may_manage=True)

    datasets = await authorized_get_principal_datasets(other.id, "read", requester_id, tenant_id)
    assert len(datasets) == 1
...
async def test_role_member_may_read_their_teams_datasets(monkeypatch):
    ...
    datasets = await authorized_get_principal_datasets(role.id, "read", requester_id, tenant_id)
    assert len(datasets) == 1
...
async def test_non_member_without_user_management_is_denied(monkeypatch):
    requester_id, tenant_id = uuid4(), uuid4()
    role = FakePrincipal("role", tenant_id=tenant_id, name="engineering")
    _patch(monkeypatch, role, role_names=["design"], may_manage=False)

    with pytest.raises(PermissionDeniedError):
        await authorized_get_principal_datasets(role.id, "read", requester_id, tenant_id)
...
async def test_role_of_another_tenant_is_not_found(monkeypatch):
    ...
    _patch(monkeypatch, foreign_role, role_names=["engineering"], may_manage=True)

    with pytest.raises(RoleNotFoundError):
        await authorized_get_principal_datasets(foreign_role.id, "read", requester_id, tenant_id)
...
async def test_tenant_datasets_readable_only_from_inside_that_tenant(monkeypatch):
    ...
    datasets = await authorized_get_principal_datasets(tenant_id, "read", requester_id, tenant_id)
    assert len(datasets) == 1
...
async def test_another_tenants_datasets_are_denied(monkeypatch):
    ...
    _patch(monkeypatch, other_tenant, datasets=[FakeDataset(tenant_id)], may_manage=True)

    with pytest.raises(PermissionDeniedError):
        await authorized_get_principal_datasets(other_tenant.id, "read", requester_id, tenant_id)
...
async def test_unsupported_principal_type_is_denied(monkeypatch):
    requester_id, tenant_id = uuid4(), uuid4()
    _patch(monkeypatch, FakePrincipal("principal"), may_manage=True)

    with pytest.raises(PermissionDeniedError):
        await authorized_get_principal_datasets(uuid4(), "read", requester_id, tenant_id)

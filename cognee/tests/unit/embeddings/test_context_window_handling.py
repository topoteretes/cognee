...
from cognee.infrastructure.databases.vector.embeddings.context_window_handler import handle_context_window_exceeded
...
class TestContextWindowHandler:
    ...
    async def test_handle_context_window_multiple_texts(self):
        ...
        result = await handle_context_window_exceeded(mock_embed, ["text1", "text2", "text3", "text4"])
...
class TestLiteLLMEmbeddingEngine:
    ...
    async def test_litellm_context_window_handling(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            ...
            with patch('litellm.aembedding', new_callable=AsyncMock, side_effect=litellm.exceptions.ContextWindowExceededError(
                model="text-embedding-3-large",
                llm_provider="openai",
                message="Context window exceeded"
            )):
                with patch('cognee.infrastructure.databases.vector.embeddings.context_window_handler.handle_context_window_exceeded', new_callable=AsyncMock, return_value=[[0.1, 0.2]]) as mock_handler:
...
class TestFastembedEmbeddingEngine:
    ...
    async def test_fastembed_context_window_handling(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            ...
            with patch('cognee.infrastructure.databases.vector.embeddings.FastembedEmbeddingEngine.TextEmbedding') as mock_text_embedding:
                mock_text_embedding.return_value = MagicMock()
                engine = FastembedEmbeddingEngine(model="sentence-transformers/all-MiniLM-L6-v2", dimensions=384)

                # Mock the embed method to raise context window error
                with patch.object(engine.embedding_model, 'embed', side_effect=Exception("Context window exceeded")):
                    with patch('cognee.infrastructure.databases.vector.embeddings.context_window_handler.handle_context_window_exceeded', new_callable=AsyncMock, return_value=[[0.1, 0.2]]) as mock_handler:
                        result = await engine.embed_text(["test"])
                        assert result == [[0.1, 0.2]]
                        mock_handler.assert_awaited_once()
    ...
    async def test_fastembed_context_error_detection(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            with patch('cognee.infrastructure.databases.vector.embeddings.FastembedEmbeddingEngine.TextEmbedding') as mock_text_embedding:
                ...
                with patch.object(engine.embedding_model, 'embed', side_effect=Exception("token limit exceeded")):
                    with patch('cognee.infrastructure.databases.vector.embeddings.context_window_handler.handle_context_window_exceeded', new_callable=AsyncMock, return_value=[[0.1, 0.2]]) as mock_handler:
                        result = await engine.embed_text(["test"])
                        assert result == [[0.1, 0.2]]
                        mock_handler.assert_awaited_once()
class TestOllamaEmbeddingEngine:
    ...
    async def test_ollama_context_window_handling(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            with patch('cognee.infrastructure.databases.vector.embeddings.OllamaEmbeddingEngine.HuggingFaceTokenizer') as mock_tokenizer:
                mock_tokenizer.return_value = MagicMock()
                engine = OllamaEmbeddingEngine(dimensions=2)

                # Mock _get_embedding to raise context window error
                with patch.object(engine, '_get_embedding', new_callable=AsyncMock, side_effect=ValueError("context window exceeded for embedding model")):
                    with patch('cognee.infrastructure.databases.vector.embeddings.context_window_handler.handle_context_window_exceeded', new_callable=AsyncMock, return_value=[[0.1, 0.2]]) as mock_handler:
                        result = await engine.embed_text(["test"])
                        assert result == [[0.1, 0.2]]
                        mock_handler.assert_awaited_once()
    ...
    async def test_ollama_context_error_detection(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            with patch('cognee.infrastructure.databases.vector.embeddings.OllamaEmbeddingEngine.HuggingFaceTokenizer') as mock_tokenizer:
                ...
                with patch.object(engine, '_get_embedding', new_callable=AsyncMock, side_effect=ValueError("context length exceeded")):
                    with patch('cognee.infrastructure.databases.vector.embeddings.context_window_handler.handle_context_window_exceeded', new_callable=AsyncMock, return_value=[[0.1, 0.2]]) as mock_handler:
                        result = await engine.embed_text(["test"])
                        assert result == [[0.1, 0.2]]
                        mock_handler.assert_awaited_once()
...
class TestIntegration:
    ...
    async def test_all_engines_consistent_handling(self):
        ...
        with patch.dict(os.environ, {'MOCK_EMBEDDING': 'false'}):
            engines = [
                (LiteLLMEmbeddingEngine(dimensions=2), 'litellm.aembedding', litellm.exceptions.ContextWindowExceededError(
                    model="text-embedding-3-large",
                    llm_provider="openai",
                    message="Context window exceeded"
                )),

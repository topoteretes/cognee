...
class TestValidateSessionParams:
    ...
    def test_valid_params(self):
        ...
        SessionManager._validate_session_params(
    ...
    def test_empty_session_id_raises(self):
        ...
        with pytest.raises(SessionParameterValidationError) as exc_info:
            SessionManager._validate_session_params(
                user_id="00000000-0000-0000-0000-000000000001", session_id=""
            )
    ...
    def test_non_uuid_user_id_raises(self):
        """A non-UUID user_id raises instead of creating a ghost session.

        Cache writes would succeed with any string, but the lifecycle row,
        dataset binding, listing, and cleanup all key on a UUID — letting a
        non-UUID through produces a session invisible to all of them.
        """
        with pytest.raises(SessionParameterValidationError) as exc_info:
            SessionManager._validate_session_params(user_id="agent-7", session_id="s1")
        assert "UUID" in exc_info.value.message

    def test_uuid_object_user_id_is_accepted(self):
        from uuid import uuid4

        SessionManager._validate_session_params(user_id=uuid4(), session_id="s1")

    def test_empty_qa_id_raises(self):
        """Empty qa_id raises when provided."""
        with pytest.raises(SessionParameterValidationError) as exc_info:
            SessionManager._validate_session_params(
                user_id="00000000-0000-0000-0000-000000000001", session_id="s1", qa_id=""
            )
    ...
    def test_valid_last_n(self):
        ...
        SessionManager._validate_session_params(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1", last_n=5
        )
        SessionManager._validate_session_params(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1", last_n=1
        )
    ...
    def test_invalid_last_n_zero_raises(self):
        ...
        with pytest.raises(SessionParameterValidationError) as exc_info:
            SessionManager._validate_session_params(
                user_id="00000000-0000-0000-0000-000000000001", session_id="s1", last_n=0
            )
...
class TestSessionManager:
    ...
    async def test_add_qa_session_id_none_uses_default(self, sm, mock_cache):
        ...
        qa_id = await sm.add_qa(
            user_id="00000000-0000-0000-0000-000000000001", question="Q", context="C", answer="A"
        )
    ...
    async def test_get_session_invalid_last_n_raises(self, sm):
        ...
        with pytest.raises(SessionParameterValidationError):
            await sm.get_session(
                user_id="00000000-0000-0000-0000-000000000001", last_n=0, session_id="s1"
            )
        with pytest.raises(SessionParameterValidationError):
            await sm.get_session(
                user_id="00000000-0000-0000-0000-000000000001", last_n=-1, session_id="s1"
            )
    ...
    async def test_get_session_calls_cache(self, sm, mock_cache):
        ...
        mock_cache.get_all_qa_entries.assert_called_once_with(
            "00000000-0000-0000-0000-000000000001", "s1"
        )
    ...
    async def test_get_agent_trace_session_calls_cache(self, sm, mock_cache):
        ...
        mock_cache.get_agent_trace_session.assert_called_once_with(
            "00000000-0000-0000-0000-000000000001", "s1", last_n=None
        )
    ...
    async def test_get_agent_trace_session_unavailable_returns_empty(self, sm_unavailable):
        ...
        assert (
            await sm_unavailable.get_agent_trace_session(
                user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
            )
            == []
        )
    ...
    async def test_get_agent_trace_feedback_calls_cache(self, sm, mock_cache):
        ...
        feedback = await sm.get_agent_trace_feedback(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )
        assert feedback == [
            "plan_trip succeeded.",
        ...
        mock_cache.get_agent_trace_feedback.assert_called_once_with(
            "00000000-0000-0000-0000-000000000001", "s1", last_n=None
        )
    ...
    async def test_get_agent_trace_feedback_passes_last_n_to_cache(self, sm, mock_cache):
        ...
        mock_cache.get_agent_trace_feedback.assert_called_once_with(
            "00000000-0000-0000-0000-000000000001", "s1", last_n=1
        )
    ...
    async def test_get_agent_trace_feedback_unavailable_returns_empty(self, sm_unavailable):
        ...
        assert (
            await sm_unavailable.get_agent_trace_feedback(
                user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
            )
            == []
        )
    ...
    async def test_get_agent_trace_count_calls_cache(self, sm, mock_cache):
        ...
        count = await sm.get_agent_trace_count(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )

        assert count == 3
        mock_cache.get_agent_trace_count.assert_called_once_with(
            "00000000-0000-0000-0000-000000000001", "s1"
        )
    ...
    async def test_get_agent_trace_count_unavailable_returns_zero(self, sm_unavailable):
        ...
        assert (
            await sm_unavailable.get_agent_trace_count(
                user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
            )
            == 0
        )
    ...
    async def test_update_qa_calls_cache_and_reindexes_when_text_changes(
        ...
        mock_cache.get_qa_entries_by_ids.assert_awaited_once_with(
            "00000000-0000-0000-0000-000000000001", "s1", ["q1"]
        )
    ...
    async def test_delete_feedback_calls_cache(self, sm, mock_cache):
        ...
        ok = await sm.delete_feedback(
            user_id="00000000-0000-0000-0000-000000000001", qa_id="q1", session_id="s1"
        )
        assert ok is True
    ...
    async def test_delete_session_calls_cache_and_deletes_vectors(
        ...
        ok = await sm.delete_session(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )
        assert ok is True
        mock_cache.delete_session.assert_called_once_with(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )
...
class TestSessionContextEntryValidation:
    ...
    async def test_get_session_context_entries_fail_open_on_cache_error(self, sm_failing_cache):
        ...
        result = await sm_failing_cache.get_session_context_entries(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )
        assert result == []
    ...
    async def test_delete_session_context_fail_open_on_cache_error(self, sm_failing_cache):
        """Cache runtime failures stay fail-open: returns False, never raises."""
        result = await sm_failing_cache.delete_session_context(
            user_id="00000000-0000-0000-0000-000000000001", session_id="s1"
        )
        assert result is False

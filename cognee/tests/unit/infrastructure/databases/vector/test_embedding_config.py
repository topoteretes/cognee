...
def test_engine_key_fallback_reuses_key_for_same_provider():
    assert (
        _get_embedding_api_key(None, "same-provider-key", "openai", "openai") == "same-provider-key"
    )
...
def test_engine_key_fallback_prefers_explicit_embedding_key():
    assert (
        _get_embedding_api_key("embedding-key", "llm-key", "openai", "anthropic") == "embedding-key"
    )

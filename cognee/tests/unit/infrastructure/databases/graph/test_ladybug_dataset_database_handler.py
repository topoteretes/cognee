...
async def test_delete_dataset_does_not_touch_lock_when_shared_lock_disabled(monkeypatch):
    """Default (SHARED_LADYBUG_LOCK unset/false): no lock is taken, but
    eviction and file removal still happen -- behavior is unchanged, not
    just "the lock call didn't fire"."""
    monkeypatch.setattr(
    ...
    aevict_mock = AsyncMock(return_value=0)
    monkeypatch.setattr(
        handler_module, "graph_engine_cache", SimpleNamespace(aevict_for_database=aevict_mock)
    )
    file_storage = _fake_file_storage()
    dataset_database = _dataset_database()
    # Only the main file "exists"; -lock/-wal companions don't, so remove()
    # is called exactly once and this assertion isn't order-of-calls-fragile.
    file_storage.is_file = AsyncMock(
        side_effect=lambda name: name == dataset_database.graph_database_name
    )
    monkeypatch.setattr(handler_module, "get_file_storage", lambda _path: file_storage)
    ...
    aevict_mock.assert_awaited_once_with(dataset_database.graph_database_name)
    file_storage.remove.assert_awaited_once_with(dataset_database.graph_database_name)

...
def test_ladybug_version_mapping_covers_0_19_storage_code():
    ...
    # database each wheel actually created), so the mapping holds the latest
    # patch of the line.
    assert ladybug_version_mapping[43] == "0.19.1"


def test_read_ladybug_storage_version_0_19_code(tmp_path):
    _write_catalog_kz(str(tmp_path), 43, magic=b"LBUG")
    assert read_ladybug_storage_version(str(tmp_path)) == "0.19.1"

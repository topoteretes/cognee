...
# 假设 TextDocument 和 DocumentChunk 现在支持 importance_weight
...
@pytest.mark.asyncio
async def test_TextDocument_and_ImportanceWeight(
        mock_engine, input_file, chunk_size, importance_weight, expected_weight
):
    ...
    async for ground_truth, paragraph_data in async_gen_zip(
        ...
        assert ground_truth["word_count"] == paragraph_data.chunk_size, (
            f'Word count mismatch: {ground_truth["word_count"] = } != {paragraph_data.chunk_size = }'
        )
        assert ground_truth["len_text"] == len(paragraph_data.text), (
            f'Text length mismatch: {ground_truth["len_text"] = } != {len(paragraph_data.text) = }'
        )
        assert ground_truth["cut_type"] == paragraph_data.cut_type, (
            f'Cut type mismatch: {ground_truth["cut_type"] = } != {paragraph_data.cut_type = }'
        )

...
def from_basic_edge(cls, edge: Edge) -> "GraphEdge":
    ...
    return cls(
        id=edge_id,
        source_node_id=UUID(edge.source_node_id)
        if isinstance(edge.source_node_id, str)
        else edge.source_node_id,
        target_node_id=UUID(edge.target_node_id)
        if isinstance(edge.target_node_id, str)
...
def from_basic_graph(cls, graph: KnowledgeGraph) -> "LayeredKnowledgeGraphDP":
    ...
    layered_graph = cls.create_empty(name=graph.name, description=graph.description)

    # Create a single base layer
    base_layer = GraphLayer.create(
        name="Base Layer", description="Base layer created from basic graph", layer_type="base"
    )
...
def get_cumulative_layer_graph(self, layer_id):
    ...
    ]  # Fixed: Using all_edges instead of edges
...
class TestGraphNode(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        node = GraphNode.create(name="Test Node", node_type="TestType", description="A test node")
...
class TestGraphEdge(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        edge = GraphEdge.create(
            source_node_id=source_id, target_node_id=target_id, relationship_name="TEST_RELATION"
        )
...
class TestGraphLayer(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        layer = GraphLayer.create(name="Test Layer", description="A test layer", layer_type="test")
...
class TestLayeredKnowledgeGraphDP(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.graph = LayeredKnowledgeGraphDP.create_empty(
            name="Test Graph", description="A test graph"
        )

        # Create layers
        self.base_layer = GraphLayer(
            id=uuid4(), name="Base Layer", description="Base layer for testing", layer_type="base"
        )
        ...
        self.node1 = GraphNode(
            id=uuid4(), name="Node 1", node_type="TestNode", description="First test node"
        )

        self.node2 = GraphNode(
            id=uuid4(), name="Node 2", node_type="TestNode", description="Second test node"
        )

        self.node3 = GraphNode(
            id=uuid4(), name="Node 3", node_type="EnrichmentNode", description="Enrichment node"
        )
        ...
        self.edge2 = GraphEdge.create(
            source_node_id=self.node3.id, target_node_id=self.node1.id, relationship_name="ENRICHES"
        )
    ...
    def test_creation(self):
        ...
        graph = LayeredKnowledgeGraphDP.create_empty(
            name="Empty Graph", description="An empty graph for testing"
        )
...
class TestIntegrationWithExisting(unittest.TestCase):
    ...
    def test_conversion_from_basic(self):
        ...
        node1 = Node(
            id=str(uuid4()), name="Basic Node 1", type="TestType", description="First basic node"
        )

        node2 = Node(
            id=str(uuid4()), name="Basic Node 2", type="TestType", description="Second basic node"
        )

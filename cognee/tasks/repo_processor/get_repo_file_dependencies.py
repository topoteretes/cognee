...
async def get_source_code_files(repo_path: str, excluded_paths: Optional[List[str]] = None) -> List[str]:
    """
...
async def get_repo_file_dependencies(
    ...
    - detailed_extraction (bool): Whether to extract detailed dependencies.
    - excluded_paths (Optional[List[str]]): List of path patterns to exclude.

    Yields:
    -------
    - Repository object
    - CodeFile objects with dependency information
    """
    if not os.path.exists(repo_path):

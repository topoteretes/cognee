...
def save_data_to_storage(data: Union[BinaryIO, Document, ImageDocument, str], dataset_name) -> list[str]:
    ...
    # Check if any data point needs to be transformed to supported data format
    for i in range(len(data)):
        if type(data[i]) == Document or type(data[i]) == ImageDocument:
            data[i] = from_llama_index_format(data[i])

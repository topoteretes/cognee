...
async def extract_layered_graph_from_content(content: str, 
                                            layer_config: Optional[List[Dict[str, Any]]] = None, 
                                            base_model: Type[BaseModel] = KnowledgeGraph) -> LayeredKnowledgeGraphDP:
    """
    ...
    layered_graph = LayeredKnowledgeGraphDP.create_empty(
        name="Extracted Layered Graph",
        description="Graph extracted from content with layers"
    )
...
async def extract_layered_graph_from_data(data_points: List[DataPoint], 
                                         layer_config: Optional[List[Dict[str, Any]]] = None,
                                         base_model: Type[BaseModel] = KnowledgeGraph) -> List[DataPoint]:
    """
    ...
    for data_point in data_points:
        ...
        layered_graph = await extract_layered_graph_from_content(
            text, layer_config, base_model
        )
...
async def store_layered_graphs(data_points: List[DataPoint]) -> List[DataPoint]:
    ...
    # Process each data point
    for data_point in data_points:
        if hasattr(data_point, "layered_graph") and isinstance(data_point.layered_graph, LayeredKnowledgeGraphDP):
...
async def enrich_layered_graph(graph: LayeredKnowledgeGraphDP, 
                              enrichment_type: str, 
                              content: Optional[str] = None,
                              parent_layer_ids: Optional[List[UUID]] = None) -> LayeredKnowledgeGraphDP:
    """
...
async def process_layered_graphs_with_pipeline(data_points: List[DataPoint], 
                                              pipeline_config: List[Dict[str, Any]]) -> List[DataPoint]:
    """

...
def _is_recall_eligible(entry: SessionQAEntry) -> bool:
    memify_metadata = entry.memify_metadata
    if (
        isinstance(memify_metadata, dict)
        and memify_metadata.get(MEMIFY_METADATA_FREQUENCY_WEIGHTS_APPLIED_KEY) is True
    ):
        return False
    ...
    # Frequency tracks recall usage, so unrated QAs still count when retrieval
    # recorded concrete graph element IDs.
    has_node_ids = isinstance(node_ids, list) and any(
        isinstance(node_id, str) for node_id in node_ids
    )
    has_edge_ids = isinstance(edge_ids, list) and any(
...
async def extract_recall_qas(data, session_ids: Optional[List[str]] = None):
    """
    Read provided sessions and yield recall QAs eligible for frequency weight updates.
    """
    if (
        not isinstance(session_ids, list)
        or not session_ids
        ...
        raise CogneeValidationError(
            message="session_ids must be provided for extract_recall_qas",
            log=False,
        )

    if not data or data == [{}]:
        logger.info("Extracting recall QAs from session cache")
    ...
    for session_id in session_ids:
        ...
        for entry in entries:
            if not isinstance(entry, SessionQAEntry) or not _is_recall_eligible(entry):
                continue

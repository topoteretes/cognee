...
async def _resolve_session_dataset(dataset_ref, user):
    """Resolve and authorize the single dataset a session write targets.

    Session entries always target exactly one dataset so its UUID can scope
    the session (per-dataset default IDs, lifecycle attribution, DB context).
    """
...
async def _add_to_session(session_id: str, data, user, *, dataset):
    """Add a Q&A entry to the session cache, scoped to ``dataset``.
    ...
    # Session QA indexing uses the vector engine — select the dataset's
    # database context so session vectors land next to the dataset's data.
    ...
    *,
    ...
    if isinstance(entry, SkillRunEntry):
        ...
        run, dataset = await remember_skill_run_entry(
            entry,
            # The downstream dataset resolver accepts a name or UUID.
    ...
    # QA writes index vectors and traces can trigger context extraction —
    # keep both in the dataset's database context.
    async with set_database_global_context_variables(dataset.id, dataset.owner_id):
    ...
    **kwargs,
    ...
    if session_id:
        ...
        result = RememberResult(
            ...
            dataset_id=str(session_dataset.id),
            session_ids=[session_id],
        )
        result.elapsed_seconds = time.monotonic() - result._started_at

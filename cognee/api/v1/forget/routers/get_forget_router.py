...
class ForgetPayloadDTO(InDTO):
    ...
    dataset_name: Optional[str] = Field(default=None, examples=[""])
...
def get_forget_router() -> APIRouter:
    ...
    async def forget_endpoint(
        ...
        - Set `dataset_name` or `dataset_id` alone to delete an entire dataset.
        - Set `dataset_name`/`dataset_id` + `data_id` to delete a single item.
        - Set `dataset_name`/`dataset_id` + `memory_only: true` to clear memory
          (graph + vector), preserving raw files so the dataset can be re-cognified.
        - Set `dataset_name`/`dataset_id` + `data_id` + `memory_only: true` to clear memory
        ...
        try:
            ...
            return JSONResponse(
                status_code=422,
                content={
                    "error": "Invalid request parameters. Specify dataset_name or dataset_id, data_id+dataset, or everything=True."
                },
            )

...
class ResponseGraphEntry(SearchResult):
...
async def _search_session(
    ...
    for _, entry in scored[:top_k]:
        results.append(ResponseQAEntry(entry))
    ...
    with new_span("cognee.api.recall") as span:
        ...
        async def _run_session() -> list[RecallResponse]:
            if not session_id:
                return []
            return [
                RecallResponse(type=r)
                for r in await _search_session(
        ...
        async def _run_graph_context() -> list[RecallResponse]:
            ...
            return [
                RecallResponse(type=r)
                for r in await _fetch_graph_context(session_id=session_id, user=user)
            ]
        ...
        async def _run_graph() -> list[RecallResponse]:
            ...
            for r in graph_results:
                tagged.append(RecallResponse(type=ResponseGraphEntry(r)))
            return tagged

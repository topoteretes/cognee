...
def get_llm_router() -> APIRouter:
    ...
    async def infer_schema(
        ...
        ## Request Parameters
        - **data** (List[UploadFile]): Files whose parsed contents are sampled for schema
          inference; required when no text is supplied.
        - **parameters** (str): JSON string of additional kwargs forwarded to LLMGateway. Defaults
          to '{}'.
        - **text** (str): Raw sample text analyzed for schema inference; required when no
          files are uploaded.
        """
        send_telemetry(

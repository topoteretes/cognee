...
from cognee.modules.visualization.embedding_join import fetch_node_embeddings
...
async def _semantic_payload(pre):
    ...
    Returns ``(positions, clusters)`` or ``(None, None)`` when embeddings can't be
    fetched — in which case the semantic tab shows a friendly empty state.
    """
    try:
        embeddings = await fetch_node_embeddings(pre.nodes)
    except Exception as exc:
        logger.warning("Semantic map: embedding fetch failed (%s); tab shows empty state.", exc)
        return None, None
    if not embeddings:
        return None, None
    positions = semantic_layout.compute_positions(pre.nodes, pre.links, embeddings)
    clusters = compute_clusters(pre.nodes, embeddings)

...
class LayeredKnowledgeGraphDP(DataPoint):
    ...
    def add_layer(self, layer: GraphLayer) -> None:
        ...
        if self._adapter:
            ...
            try:
                ...
                asyncio.create_task(self._adapter._graph_db.add_edge(
                    str(self.id),
                    str(layer.id),
                    "CONTAINS_LAYER",
    ...
    async def from_database(cls, graph_id: UUID, adapter):
        ...
        if hasattr(adapter._graph_db, 'extract_node'):
            ...
            logger.debug(f"Graph loaded in memory: {getattr(adapter._graph_db, 'graph') is not None}")
            
            if hasattr(adapter._graph_db, 'graph') and adapter._graph_db.graph:
                logger.debug(f"Graph has node {str(graph_id)}: {adapter._graph_db.graph.has_node(str(graph_id))}")
                ...
                for node_id in adapter._graph_db.graph.nodes():
                    ...
                    node_count += 1
                    if node_count > 20:
                        logger.debug(f"  ... and {len(adapter._graph_db.graph.nodes()) - 20} more nodes")
                        break
        ...
        try:
                    ...
                    for layer_data in layers_data:
                        ...
                        if layer_node:
                            layer = GraphLayer(
                                ...
                                parent_layers=[UUID(parent_id) for parent_id in layer_node.get('parent_layers', [])],
                ...
                # For NetworkXAdapter which uses direct graph access
                            ...
                            if nx_graph.has_edge(str(graph_id), node_id):
                                ...
                                for key, edge_data in edges.items():
                                    if key == 'CONTAINS_LAYER':
                                        logger.debug(f"Found layer {node_id} belonging to graph via edge {key}")
                                        layer_found = True
                                        break
                            
                            # If not found, try looking for all edges
                            if not layer_found:
                                for source, target, key, edge_data in nx_graph.edges(str(graph_id), data=True, keys=True):
                                ...
                                try:
                                    ...
                                    if 'parent_layers' in node_data:
                                        ...
                                        if isinstance(raw_parent_layers, str):
                                            import json
                                            parent_layers = [UUID(p) for p in json.loads(raw_parent_layers)]
                                    ...
                                    logger.error(f"Error creating layer from node {node_id}: {str(e)}")
                    ...
                    for node_id, node_data in nx_graph.nodes(data=True):
                        if node_data.get('metadata', {}).get('type') == 'GraphNode':
                            ...
                            for _, target, key, edge_data in nx_graph.edges(node_id, data=True, keys=True):
                                if key == 'IN_LAYER' and UUID(target) in graph.layers:
                                    ...
                                    try:
                                        ...
                                        logger.error(f"Error creating node from node {node_id}: {str(e)}")
                            ...
                            for _, target, key, edge_data in nx_graph.edges(node_id, data=True, keys=True):
                                if key == 'IN_LAYER' and UUID(target) in graph.layers:
                                    ...
                                    try:
                                        ...
                                        logger.error(f"Error creating edge from node {node_id}: {str(e)}")
                    
                    logger.debug(f"Found {edge_count} edges via direct access")
                
                else:
                    logger.warning(f"Adapter {type(adapter._graph_db).__name__} doesn't support query or direct graph access")
            ...
            logger.debug(f"Final graph contains {len(graph.layers)} layers, {len(graph.nodes)} nodes, and {len(graph.edges)} edges")
            return graph

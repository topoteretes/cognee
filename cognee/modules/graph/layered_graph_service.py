...
from cognee.shared.data_models import KnowledgeGraph, Layer, Node, Edge
...
class LayeredGraphService:
    ...
    async def merge_layers(
        ...
        builder = LayeredGraphBuilder(
            name=layered_graph.name, description=layered_graph.description
        )
        ...
        new_layer_id = builder.create_layer(
            name=new_layer_name, description=new_layer_description, layer_type=new_layer_type
        )
    ...
    async def diff_layers(
        layered_graph: LayeredKnowledgeGraph, base_layer_id: str, comparison_layer_id: str
    ) -> Dict[str, Any]:
        """
        ...
        for node_id in common_nodes:
            ...
            comparison_node = next(
                (node for node in comparison_graph.nodes if node.id == node_id), None
            )

            if base_node is not None and comparison_node is not None:
                # Check if properties differ
                if (
                    base_node.name != comparison_node.name
                    or base_node.type != comparison_node.type
                    or base_node.description != comparison_node.description
                    or base_node.properties != comparison_node.properties
                ):
        ...
        for edge_key in common_edges:
            base_edge = next(
                (
                    edge
                    for edge in base_graph.edges
                    if (edge.source_node_id, edge.target_node_id, edge.relationship_name)
                    == edge_key
                ),
                None,
            )
            comparison_edge = next(
                (
                    edge
                    for edge in comparison_graph.edges
                    if (edge.source_node_id, edge.target_node_id, edge.relationship_name)
        ...
        for layer_id in layer_ids:
            ...
            if edge_filter is not None:
                filtered_edges = [
                    ...
                    for edge in layer_graph.edges
                    if edge_filter(edge)
                    and edge.source_node_id in filtered_node_ids
                    and edge.target_node_id in filtered_node_ids
                ]
            else:
                filtered_edges = [
                    edge
                    for edge in layer_graph.edges
                    if edge.source_node_id in filtered_node_ids
                    and edge.target_node_id in filtered_node_ids
                ]
    ...
    async def analyze_layer_dependencies(layered_graph: LayeredKnowledgeGraph) -> Dict[str, Any]:
        ...
        return {
            "root_layers": roots,
            "leaf_layers": [
                layer_id for layer_id, children in reverse_dependencies.items() if not children
            ],
    ...
    async def find_nodes_by_property(
        ...
        def node_filter(node):
            if property_name == "id":
                ...
                return (
                    property_name in node.properties
                    and node.properties[property_name] == property_value
                )
    ...
    @staticmethod
    async def calculate_layer_metrics(
        layered_graph: LayeredKnowledgeGraph,

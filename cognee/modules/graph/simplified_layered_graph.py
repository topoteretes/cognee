...
class GraphNode(DataPoint):
    ...
    def create(cls, name: str, node_type: str, description: str = "", 
               properties: Dict = None, metadata: Dict = None) -> 'GraphNode':
        """
...
class GraphEdge(DataPoint):
    ...
    properties: Dict[str, Any] = Field(default_factory=dict)
    layer_id: Optional[UUID] = None
    metadata: Dict[str, Any] = Field(default_factory=lambda: {"type": "GraphEdge", "index_fields": ["edge_type"]})
    
    @classmethod
    def create(cls, source_id: UUID, target_id: UUID, edge_type: str, 
               properties: Dict = None, metadata: Dict = None) -> 'GraphEdge':
        """
...
class LayeredKnowledgeGraph(DataPoint):
                ...
                for _, target, key in nx_graph.edges(str(self.id), keys=True):
                    if key == 'CONTAINS_LAYER':
                        ...
                        if 'parent_layers' in layer_data:
                            ...
                            try:
                                if isinstance(layer_data['parent_layers'], str):
                                    ...
                                    try:
                                        import json
                                        parent_layers = [safe_uuid(p) for p in json.loads(layer_data['parent_layers'])]
                                    except json.JSONDecodeError:
                                        # Treat as a comma-separated string
                                        parent_layers = [safe_uuid(p.strip()) for p in layer_data['parent_layers'].split(',')]
                                elif isinstance(layer_data['parent_layers'], list):
                                    parent_layers = [safe_uuid(p) for p in layer_data['parent_layers']]
                                ...
                                for _, parent_target, edge_key in nx_graph.edges(target, keys=True):
                                    if edge_key == 'EXTENDS_LAYER':
                                        ...
                                        if parent_uuid:
                                            ...
                                            logger.debug(f"Found parent layer via edge: {parent_target}")
                            except Exception as e:
                        ...
                        try:
                            ...
                            layers.append(layer)
                            logger.debug(f"Created layer object: {layer.id} with {len(parent_layers)} parent layers")
                        except Exception as e:
                            logger.error(f"Error creating layer object: {e}")
        
        return layers
    
    async def add_layer(self, name: str, description: str, layer_type: str = "default", 
                       parent_layers: List[UUID] = None) -> GraphLayer:
        """
        ...
        if hasattr(self.adapter._graph_db, 'graph') and isinstance(self.adapter._graph_db.graph, nx.MultiDiGraph):
    ...
    async def add_node(self, name: str, node_type: str, properties: Dict = None, 
                    metadata: Dict = None, layer_id: UUID = None) -> GraphNode:
        """
    ...
    async def add_edge(self, source_id: UUID, target_id: UUID, edge_type: str, 
                    properties: Dict = None, metadata: Dict = None, 
                    layer_id: UUID = None) -> GraphEdge:
        """
    ...
    async def get_cumulative_layer_graph(self, layer_id: UUID) -> Tuple[List[GraphNode], List[GraphEdge]]:
        """
        ...
        def include_layer_and_parents(current_layer_id: UUID):
            ...
            logger.debug(f"Processing layer {current_layer_id} with {len(current_layer.parent_layers)} parents")

...
class LayeredGraphDBAdapter:
    ...
    async def _store_layers(self, graph: LayeredKnowledgeGraphDP) -> None:
        ...
        for layer_id, layer in graph.layers.items():
            ...
            for parent_id in layer.parent_layers:
                layer_relationship_tasks.append(
                    self._graph_db.add_edge(
                        str(layer.id),
                        str(parent_id),
                        "EXTENDS_LAYER",
    ...
    async def _add_edge_with_adapter(
        self,
        from_node: str,
        to_node: str,
        relationship_name: str,
    ...
    async def retrieve_graph(self, graph_id: Union[str, UUID]) -> LayeredKnowledgeGraphDP:
        ...
        try:
            ...
            graph = LayeredKnowledgeGraphDP.create_empty(
                name="Retrieved Graph", description="Graph retrieved from database"
            )
            ...
            logger.debug(
                f"Retrieved {len(nodes_data)} nodes and {len(edges_data)} edges from database"
            )
            ...
            if self._adapter_type == "NetworkXAdapter":
                ...
                for source, target, rel_type, props in edges_data:
                    if (
                        isinstance(source, UUID)
                        and isinstance(target, UUID)
                        and rel_type == "CONTAINS_LAYER"
                    ):
                        if source == uuid_graph_id:
                            contains_layer_count += 1
                            logger.debug(
                                f"Found CONTAINS_LAYER relationship from graph {uuid_graph_id} to layer {target}"
                            )
                            ...
                            for node_id, node_props in nodes_data:
                                if isinstance(node_id, UUID) and node_id == layer_id:
                                    ...
                                    logger.debug(
                                        f"Found layer node: {node_id} with properties: {node_props}"
                                    )
                                    break
                ...
                logger.debug(
                    f"Found {contains_layer_count} CONTAINS_LAYER relationships, created {len(graph_layers)} layer objects"
                )
            else:
            ...
            if self._adapter_type == "NetworkXAdapter":
                ...
                for source, target, rel_type, props in edges_data:
                    ...
                    if rel_type == "EXTENDS_LAYER":
                        if isinstance(source, UUID) and isinstance(target, UUID):
                            ...
                            logger.debug(
                                f"Found EXTENDS_LAYER relationship from {child_layer_id} to {parent_layer_id}"
                            )
                            ...
                            if child_layer_id in graph_layers and parent_layer_id in graph_layers:
                                ...
                                logger.debug(
                                    f"Added parent {parent_layer_id} to child {child_layer_id}"
                                )
                ...
                for source, target, rel_type, props in edges_data:
                    ...
                    if (
                        rel_str == "EXTENDS_LAYER"
                        and source_str in graph_layers
                        and target_str in graph_layers
                    ):
            ...
            if self._adapter_type == "NetworkXAdapter":
                        ...
                        for s, t, r, p in edges_data:
                            if isinstance(s, UUID) and isinstance(t, UUID) and r == "IN_LAYER":
                                if s == node_id and str(t) in graph_layers:
                                    ...
                                    node_layer_id = t
                                    logger.debug(
                                        f"Found IN_LAYER relationship from node {node_id} to layer {node_layer_id}"
                                    )
                                    break
                ...
                logger.debug(
                    f"Found {graph_node_count} GraphNode nodes, {in_layer_count} IN_LAYER relationships, added {nodes_added} nodes to graph"
                )
            else:
                # Standard processing for other adapters
                for node_id, node_props in nodes_data:
                    if node_props.get("type", "") == "GraphNode":
                        ...
                        for source, target, rel_type, props in edges_data:
                            ...
                            if (
                                source_str == node_id_str
                                and rel_str == "IN_LAYER"
                                and target_str in graph_layers
                            ):
            ...
            if self._adapter_type == "NetworkXAdapter":
                ...
                for edge_node_id, edge_props in nodes_data:
                    if edge_props.get("type", "") == "GraphEdge":
                        graph_edge_count += 1
                        logger.debug(
                            f"Found GraphEdge: {edge_node_id} with properties: {edge_props}"
                        )
                        ...
                        for s, t, r, p in edges_data:
                            if isinstance(s, UUID) and isinstance(t, UUID) and r == "IN_LAYER":
                                if s == edge_node_id and str(t) in graph_layers:
                                    ...
                                    edge_layer_id = t
                                    logger.debug(
                                        f"Found IN_LAYER relationship from edge {edge_node_id} to layer {edge_layer_id}"
                                    )
                                    break
                ...
                logger.debug(
                    f"Found {graph_edge_count} GraphEdge nodes, {edge_in_layer_count} IN_LAYER relationships, added {edges_added} edges to graph"
                )
            else:
                # Standard processing for other adapters
                for edge_node_id, edge_props in nodes_data:
                    if edge_props.get("type", "") == "GraphEdge":
                        ...
                        edge_layer_id = None
                        edge_id_str = (
                            str(edge_node_id) if isinstance(edge_node_id, UUID) else edge_node_id
                        )
            ...
            logger.info(
                f"Successfully retrieved layered graph with ID {graph_id} with {len(graph.layers)} layers, {len(graph.nodes)} nodes, and {len(graph.edges)} edges"
            )
            return graph
    ...
    async def merge_layers(
        self,
        graph_id: Union[str, UUID],
        layer_ids: List[Union[str, UUID]],
        new_layer_name: str = "Merged Layer",
        new_layer_description: str = "Merged layer",
    ) -> UUID:
        """
        ...
        try:
            ...
            uuid_layer_ids = [
                UUID(layer_id) if isinstance(layer_id, str) else layer_id for layer_id in layer_ids
            ]
            ...
            for layer_id in uuid_layer_ids:
                if layer_id not in graph.layers:
                    raise EntityNotFoundError(
                        f"Layer with ID {layer_id} not found in graph {graph_id}"
                    )
                    ...
                    if node.layer_id == layer_id:
                        ...
                        new_node = GraphNode(
                            ...
                            metadata={
                                ...
                                "original_layer_id": str(layer_id),
                                **node.metadata,
                            },
                        )
    ...
    async def get_layer_metrics(self, layer_id: Union[str, UUID]) -> Dict[str, Any]:
        ...
        try:
            ...
            for source, target, rel_type, props in edges_data:
                ...
                for node_id, node_props in nodes_data:
                    if (
                        node_props.get("type") == "GraphNode"
                        and node_props.get("layer_id") == layer_id
                    ):
            ...
            metrics = {
                ...
                "density": relationship_count / (node_count * (node_count - 1))
                if node_count > 1
                else 0,
    ...
    async def find_cross_layer_relationships(
        self, graph_id: Union[str, UUID]
    ) -> List[Dict[str, Any]]:
        """
                        ...
                        cross_layer_relationships.append(
                            {
                                ...
                                "source_layer_name": source_layer_node.get(
                                    "name", f"Layer {source_layer}"
                                )
                                if source_layer_node
                                else f"Layer {source_layer}",
                                "target_layer_name": target_layer_node.get(
                                    "name", f"Layer {target_layer}"
                                )
                                if target_layer_node
    ...
    async def delete_graph(self, graph_id: Union[str, UUID]) -> bool:
        ...
        try:
            ...
            if self._adapter_type == "NetworkXAdapter":
                ...
                logger.info(
                    f"Successfully deleted graph with ID {graph_id} and {len(nodes_to_delete)} associated nodes"
                )
                return True
            else:
    ...
    async def extract_subgraph(
        self, graph_id: Union[str, UUID], layer_id: Union[str, UUID]
    ) -> LayeredKnowledgeGraphDP:
        """
        ...
        try:
            ...
            for edge_id, edge in graph.edges.items():
                if edge.layer_id == uuid_layer_id:
                    ...
                    if (
                        edge.source_node_id in subgraph.nodes
                        and edge.target_node_id in subgraph.nodes
                    ):

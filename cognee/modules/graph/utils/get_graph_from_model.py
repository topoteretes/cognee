...
async def get_graph_from_model(
    ...
    # Handle Edge objects differently since they don't have an id attribute
    if isinstance(data_point, Edge):
        edge_key = f"{data_point.source_node_id}_{data_point.target_node_id}_{data_point.relationship_name}"
    ...
    # If it's not an Edge, proceed as usual
    if not isinstance(data_point, Edge):
        ...
        # For Edge objects, directly add them to the edges list
        edge_key = f"{data_point.source_node_id}_{data_point.target_node_id}_{data_point.relationship_name}"
        if edge_key not in added_edges:
            edges.append(
                (
                    data_point.source_node_id,
                    data_point.target_node_id,
                    data_point.relationship_name,
                    {
                        "source_node_id": data_point.source_node_id,
                        "target_node_id": data_point.target_node_id,
                        "relationship_name": data_point.relationship_name,
                        "updated_at": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"),
                        "properties": data_point.properties or {},
                    },
                )
            )

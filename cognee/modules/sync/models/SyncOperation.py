...
class SyncOperation(Base):
    ...
    status = Column(SQLEnum(SyncStatus), default=SyncStatus.STARTED, doc="Current status of the sync operation")
    ...
    # Timing information
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), doc="When the sync was initiated")
    started_at = Column(DateTime(timezone=True), doc="When the actual sync processing began")
    completed_at = Column(DateTime(timezone=True), doc="When the sync finished (success or failure)")

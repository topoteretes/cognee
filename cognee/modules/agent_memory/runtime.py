    ...
    with new_span("cognee.agent_memory.retrieve") as span:
        ...
        try:
            ...
            results = await search(
                ...
                # An only_context search returns the user prompt cognee would send —
                # its own question framing included — which must not be pasted into the
                # agent's prompt as "memory". The verbose payload keeps the bare
                # retrieval context under context_result, so read that instead.

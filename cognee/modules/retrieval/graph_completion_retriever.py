...
class GraphCompletionRetriever(BaseRetriever):
    ...
    async def get_triplets(self, query: str) -> list:
        ...
        for subclass in subclasses:
            # Updated to safely handle both Pydantic V1 and V2 models
            # Check if metadata exists in model_fields
            if "metadata" in subclass.model_fields:
                ...
                if hasattr(metadata_field, "default") and metadata_field.default is not None:
                    # Handle PydanticUndefinedType by checking if it's a dict first

...
class BroadRetriever(CompletionRetriever):
    ...
    def count_entities(
        ...
        return CountResult(
            ...
            total=len(matching),
            units=len(entities),
            evidence=[unit.text for unit in matching[:BROAD_EVIDENCE_SHOWN]],
        )
    ...
    async def count_by_reading(self, plan: CountPlan, units: list[Unit]) -> CountResult:
        ...
        async def read(shard: list[Unit]) -> list[tuple[str, ExtractedItem]]:
            async with semaphore:
                return await self.extract(plan, shard)

        listed = [pair for pairs in await asyncio.gather(*map(read, shards)) for pair in pairs]
        ...
        items: list[ExtractedItem] = []
        for shard_id, item in listed:
            marker = (shard_id, item.unit, item.evidence.strip().lower())
            if marker not in seen:
                seen.add(marker)
                items.append(item)

        names_merged = await self.merge_name_variants(plan, items)
    ...
    async def extract(self, plan: CountPlan, shard: list[Unit]) -> list[tuple[str, ExtractedItem]]:
        ...
        return [(shard[0].id, item) for item in result.items]

    async def merge_name_variants(self, plan: CountPlan, items: list[ExtractedItem]) -> bool:
        """Rewrite every name-valued field to one canonical spelling, in place.

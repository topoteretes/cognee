...
async def create_sample_layered_graph() -> LayeredKnowledgeGraphDP:
    ...
    logger.info(f"Created graph with {len(graph.layers)} layers, {len(graph.nodes)} nodes, and {len(graph.edges)} edges")
    return graph
...
def validate_retrieved_graph(original: LayeredKnowledgeGraphDP, retrieved: LayeredKnowledgeGraphDP):
    ...
    for layer_id, layer in original.layers.items():
        ...
        assert set(layer.parent_layers) == set(retrieved_layer.parent_layers), f"Layer {layer_id} parent layers do not match"
    ...
    for edge_id, edge in original.edges.items():
        ...
        assert edge.source_node_id == retrieved_edge.source_node_id, f"Edge {edge_id} source nodes do not match"
        assert edge.target_node_id == retrieved_edge.target_node_id, f"Edge {edge_id} target nodes do not match"
        assert edge.relationship_name == retrieved_edge.relationship_name, f"Edge {edge_id} relationship names do not match"

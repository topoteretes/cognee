...
class CompanyKnowledgeGraphBuilder:
    ...
    async def _add_organization_structure(self):
        ...
        teams = [
            ...
            {"name": "Accounting", "department": "Finance", "description": "Accounting team"},
            {
                "name": "FP&A",
                "department": "Finance",
                "description": "Financial planning and analysis team",
            },
        ]
        ...
        for team in teams:
            team_node = GraphNode(
                id=uuid4(), name=team["name"], node_type="Team", description=team["description"]
            )
        ...
        logger.info(
            f"Added {len(org_nodes)} nodes and {len(org_edges)} edges to organizational structure layer"
        )
    ...
    async def _add_people_layer(self):
        ...
        managers = [
            {
                ...
                "team": "Backend",
                "description": "Backend Engineering Manager",
            },
            {
                "name": "Emma Lewis",
                "title": "Engineering Manager",
                "team": "Frontend",
                "description": "Frontend Engineering Manager",
            },
            {
                "name": "Alex Rivera",
                "title": "Engineering Manager",
                "team": "DevOps",
                "description": "DevOps Engineering Manager",
            },
            {
                "name": "Jennifer Park",
            ...
            },
            {
                ...
                "title": "Engineering Manager",
                "team": "Mobile",
                "description": "Mobile Engineering Manager",
            },
            {
                "name": "Olivia Brown",
            ...
            },
            {
                ...
                "team": "Research",
                "description": "User Research Manager",
            },
            {
                "name": "Sophia Lee",
                "title": "Design Manager",
                "team": "UI",
                "description": "UI Design Manager",
            },
            {
                "name": "Ethan Davis",
                "title": "Design Manager",
            ...
            {
                "name": "Liam Scott",
                "title": "Financial Analyst Manager",
                "team": "FP&A",
                "description": "Financial Planning Manager",
            },
        ]
        ...
        for mgr_info in managers:
            ...
            mgr_edge = GraphEdge(
                source_node_id=mgr_node.id, target_node_id=team_node.id, relationship_name="LEADS"
            )
        ...
        logger.info(
            f"Added {len(people_nodes)} nodes and {len(people_edges)} edges to people layer"
        )
    ...
    async def _add_project_layer(self):
        ...
        dependencies = [
            {
                "from": "Backend API Refactoring",
                "to": "Customer Portal",
                "description": "API needed for portal",
            },
            {
                "from": "Backend API Refactoring",
                "to": "Mobile App Redesign",
                "description": "API needed for mobile",
            },
            {
                "from": "Cloud Migration",
                "to": "Data Analytics Platform",
                "description": "Platform will use new cloud infrastructure",
            },
        ]
        ...
        logger.info(
            f"Added {len(project_nodes)} nodes and {len(project_edges)} edges to projects layer"
        )
    ...
    async def _add_insights_layer(self):
        ...
        for risk in risks:
            risk_node = GraphNode(
                ...
                properties={"severity": risk["severity"], "impact": risk["impact"]},
            )
        ...
        for rec in recommendations:
            rec_node = GraphNode(
                ...
                properties={"action": rec["action"], "created_at": datetime.now().isoformat()},
            )
        ...
        logger.info(
            f"Added {len(insights_nodes)} nodes and {len(insights_edges)} edges to insights layer"
        )
...
async def analyze_layered_graph(graph: LayeredKnowledgeGraphDP):
    ...
    if "insights" in [graph._get_layer(lid).name for lid in graph.layers]:
        insights_layer_id = next(
            lid for lid in graph.layers if graph._get_layer(lid).name == "Insights"
        )
        ...
        for risk in risks:
            ...
            for edge in insights_graph.edges:
                if edge.source_node_id == risk.id and edge.relationship_name == "AFFECTS":
                    affected_project = next(
                        (n for n in insights_graph.nodes if n.id == edge.target_node_id), None
                    )
                    if affected_project:
...
async def export_visualization_data(graph: LayeredKnowledgeGraphDP, filename: str):
    ...
    for layer_id in graph.layers:
        for node in graph.get_layer_nodes(layer_id):
            node_layer_map[str(node.id)] = {
                "layer_id": str(layer_id),
                "layer_name": graph.get_layer(layer_id).name,
            }
    ...
    if layer_ids:
        ...
        for node in cumulative_graph.nodes:
            layer_info = node_layer_map.get(
                node.id, {"layer_id": "unknown", "layer_name": "Unknown"}
            )
    ...
    logger.info(
        f"Exported visualization data to {filename} ({len(viz_data['nodes'])} nodes, {len(viz_data['edges'])} edges)"
    )

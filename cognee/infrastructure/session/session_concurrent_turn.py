...
class SessionTurnContext(BaseModel):
    ...
    # Guard for the implicit-rating bridge: an explicit score is never overwritten.
    previous_feedback_score: int | None = None
    ...
    *,
    ...
    try:
        ...
        return SessionTurnContext(
            ...
            previous_feedback_score=previous.get("feedback_score"),
...
async def analyze_turn(snapshot: SessionTurnContext) -> SessionTurnAnalysis:
    ...
    recent_entries = [
        ...
        for qa_id, question, answer in snapshot.recent_qas
    ]
    try:
        return await asyncio.wait_for(
            analyze_turn_for_session_context(
                ...
                recent_turns=render_recent_turns(recent_entries)
...
async def commit_turn(
    ...
    *,
    ...
    """
    # Feedback attribution: prefer QAs the model explicitly referenced from the
    # tagged recent-turns block, falling back to the previous answer (same policy
    # as the sequential path).
    recent_qa_ids = {qa_id for qa_id, _question, _answer in snapshot.recent_qas if qa_id}
    referenced_qa_ids = [
        qa_id for qa_id in (analysis.referenced_qa_ids or []) if qa_id in recent_qa_ids
    ]
    await apply_session_turn_analysis(
        ...
        or ([snapshot.previous_qa_id] if snapshot.previous_qa_id else []),
        served_ids=[entry_id for entry_id, _content in snapshot.previous_served_context],
    )

    # Implicit-rating bridge (kept from the sequential path): turn analysis can score
    # the previous answer so non-rating users still feed the feedback-weights stage.
    # The snapshot only carries the previous entry's explicit score, so only that QA
    # is rated here — never a clobber, since an existing score short-circuits.
    if not referenced_qa_ids or referenced_qa_ids[0] == snapshot.previous_qa_id:
        await apply_implicit_answer_rating(
            session_manager,
            user_id=user_id,
            session_id=session_id,
            previous_entry={
                "qa_id": snapshot.previous_qa_id,
                "feedback_score": snapshot.previous_feedback_score,
            },

...
def calculate_backoff(attempt, initial_backoff=DEFAULT_INITIAL_BACKOFF, 
                     backoff_factor=DEFAULT_BACKOFF_FACTOR, jitter=DEFAULT_JITTER):
    """
...
def sleep_and_retry_sync(max_retries=DEFAULT_MAX_RETRIES, 
...
def sleep_and_retry_async(max_retries=DEFAULT_MAX_RETRIES, 
                          initial_backoff=DEFAULT_INITIAL_BACKOFF,
                          backoff_factor=DEFAULT_BACKOFF_FACTOR,
                          jitter=DEFAULT_JITTER):
    """
    ...
    def hit_limit(self) -> bool:
        ...
        with self.lock:
            ...
            if len(self.request_times) >= self.requests_limit:
                logger.info(f"Rate limit hit: {len(self.request_times)} requests in the last {self.interval_seconds} seconds")
                return True
...
def embedding_sleep_and_retry_sync(max_retries=5, base_backoff=1.0, jitter=0.5):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            disable_retries = os.environ.get("DISABLE_RETRIES", "false").lower() in ("true", "1", "yes")
            ...
            while retries <= max_retries:
                try:
                    ...
                    is_rate_limit = any(pattern in error_str.lower() for pattern in RATE_LIMIT_ERROR_PATTERNS)
                    ...
                    if is_rate_limit and retries < max_retries:
                        ...
                        backoff = base_backoff * (2 ** retries) * (1 + random.uniform(-jitter, jitter))

...
all of it already and pass through as a no-op. The definitions below are the
same frozen snapshot the initial revision (8057ae7329c2) carries — the
...
def _index(name: str, table: str, columns: list, **kw) -> sa.Index:
    return sa.Index(name, *(_FROZEN.tables[table].c[column] for column in columns), **kw)


def _define_frozen_schema() -> None:
    """Populate ``_FROZEN`` (deferred: ``_enum`` needs the live bind). Alembic
    caches this module per process, so a second database migrated in the same
    process must not redefine the tables."""
    if _FROZEN.tables:
        return
...
def _missing_column(column: sa.Column, dialect: str) -> sa.Column:
    ...
    for fk in column.foreign_keys:
        ...
        if name is None and dialect == "sqlite":
...
def upgrade() -> None:
    bind = op.get_bind()
    _define_frozen_schema()
    inspector = sa.inspect(bind)
    existing = set(inspector.get_table_names())

    # Plan first; change nothing until the plan is known to be safe.
    missing_tables = [table for table in _FROZEN.sorted_tables if table.name not in existing]
    missing_columns: dict[sa.Table, list[sa.Column]] = {}
    for table in _FROZEN.sorted_tables:

"""Run BEAM-10M benchmark evaluation across conversations.

Usage:
    uv run python cognee/eval_framework/run_beam_10m_eval.py
"""
...
NUM_CONVERSATIONS = 10  # BEAM-10M has 10 conversations


def _make_eval_params(conversation_index: int) -> dict:
    """Create eval params for a single BEAM-10M conversation run."""
    return EvalConfig(
        benchmark="BEAM-10M",
...
async def run_single_conversation(conversation_index: int) -> dict:
    """Run the full eval pipeline for one BEAM-10M conversation."""
    logger.info(
        f"=== BEAM-10M Evaluation: conversation {conversation_index} / {NUM_CONVERSATIONS - 1} ==="
    )
...
async def main():
    ...
    for conv_idx in range(NUM_CONVERSATIONS):
        ...
        metrics_path = f"beam10m_metrics_conv{conv_idx}.json"
    ...
    with open("beam10m_combined_results.json", "w") as f:
        json.dump(combined, f, indent=2)

    # Print summary
    logger.info(f"\n=== BEAM-10M Results (averaged over {NUM_CONVERSATIONS} conversations) ===")

...
Read them in order. Flow 1 gets you to the fork; flows 3 and 4 are the two sides of it;
flow 2 is a mechanism both of them lean on; flow 5 is what happens after the session ends.
...
```
   ...
   sequential mode                 concurrent mode  (default)
    ...
    ┌────┴────┐                   ┌────┴────┐
    │ FLOW 3  │                   │ FLOW 4  │──uses──►┌────────┐
    │ 2 calls │                   │ 1 call  │         │ FLOW 2 │ merging
    │ in a row│                   │ + 1 free│         └────────┘ two retrievals
    └────┬────┘                   └────┬────┘
         │                             │
         └──────────┬──────────────────┘
...
**Where it starts:** a user calls `await cognee.search("...", session_id="s1")`.
**Where it ends:** one function decides whether this search runs as a concurrent turn.
...
This is the step to remember. It sets a `contextvars` variable that deeper code reads as
`session_user.get()` — without going through the call signature. Flow 4 step 2 depends on it.
...
The retriever now holds the query's settings: `top_k`, prompt paths, `response_model`, and
`session_id`.
...
Not every search goes through `get_retriever_output`. Code that holds a retriever can call it
...
# Flow 2 — Merging two retrievals into one

**Where it starts:** flow 4 has just run two retrievals and holds two result sets.
**Where it ends:** one result set, ready to be formatted into context.

Read this before flow 4 — it is called from the middle of it.
...
**Why tiers and not a score.** Appearing in both lanes is the strongest evidence available,
and it is a fact, not a number to tune. An earlier draft used Reciprocal Rank Fusion; with two
lanes at near-equal weights it degenerates — a contextual-only hit cannot outrank a
raw-only hit until the raw hit falls past rank 63. Tiering says the intended thing directly.
...
c. [`analyze_turn_for_session_context` (L45)](../cognee/infrastructure/session/feedback_detection.py)
   makes one structured LLM call returning a `SessionTurnAnalysis`:
...
d. [`apply_session_turn_analysis` (L279)](../cognee/infrastructure/session/session_turn.py)
   writes the evidence entry, applies the candidate updates to the context store, and bumps
   the helpful/harmful counters
...
[get_retriever_output L92](../cognee/modules/search/methods/get_retriever_output.py) returns
the acknowledgement immediately. No retrieval. No second LLM call. "ok, thanks" costs one call.

### 4 — Retrieve once

[line 107](../cognee/modules/search/methods/get_retriever_output.py) —
`get_retrieved_objects(query=effective_query)`, using the **rewritten** query from step 2.
Then `update_node_access_timestamps`, then `get_context_from_objects`.
...
b. `build_active_context_block_safe` — renders the guidance block **and reports which entry
   ids it served**
...
Flow 1 step 6 succeeded. Resolve the session:
...
- `select_session_history` — the same helper flow 3 calls in step 5a
- `build_active_context_block_safe` — likewise, when auto-feedback is on

then, if the previous turn served any context entries, `load_served_context_payload`.

The result is a frozen
[`SessionTurnSnapshot` (L4)](../cognee/infrastructure/session/session_search_models.py).

**Why frozen, why once.** Two lanes are about to read it concurrently. Reading the cache twice
would be slower and could return two different pictures of the same session.
...
makes the identical `analyze_turn_for_session_context` call flow 3 makes in step 2c, wrapped in
a 30-second `wait_for` and failing open to an empty analysis.
...
b. If the rewrite came back identical to the raw query — no history yet — only one lane runs.
   Otherwise both `get_retrieved_objects` calls go out together with `return_exceptions=True`.
   One lane failing is survivable; both failing re-raises —
   [L150](../cognee/modules/retrieval/session_search.py).

c. `retriever.merge_retrieved_objects(raw, contextual)` → **this is flow 2**

d. `get_context_from_objects(query=raw_query, ...)` — note **raw**, not the rewrite. The
   rewrite exists to widen retrieval; the context should still be built around the question the
   user actually asked.
...
[`complete_turn` (L143)](../cognee/infrastructure/session/session_concurrent_turn.py)
uses the caller's own prompts and `response_model`, and appends one rule to the system prompt.
...
concurrent, skipping the answer would save no wall-clock time anyway.

### 7 — Both lanes have landed: commit

[`commit_turn` (L183)](../cognee/infrastructure/session/session_concurrent_turn.py) calls
`apply_session_turn_analysis` — the very function flow 3 calls at step 2d — and then `add_qa`.
...
to whichever door in flow 1 step 5 or step 8 asked.
...
**Where it ends:** accepted learnings are documents in the knowledge graph.
...
performs the two reads above. Context entries are kept only when
`harmful_count == 0` **and** `confidence >= MIN_GATE_CONFIDENCE`.
...
a. two vector searches — prior lessons in `DocumentChunk_text` scoped to the `session_learnings`
   node set (*is this already known?*), and an entity glossary from `Entity_name`
...
2. [`merge_results.py`](../cognee/modules/retrieval/utils/merge_results.py) — self-contained, no session concepts (flow 2)
3. [`hybrid/merge.py`](../cognee/modules/retrieval/hybrid/merge.py) — the same idea over a dict of channels
4. [`session_concurrent_turn.py`](../cognee/infrastructure/session/session_concurrent_turn.py) — four functions, in call order (flow 4 steps 3, 4, 6, 7)
...
6. the retriever diffs — two to four lines each (flow 2 step 6)

...
- **ONNX Runtime backend, no torch** (`GLINER_BACKEND=onnx`, default `torch`): extraction runs with neither torch nor gliner2 installed — only core dependencies (onnxruntime, numpy, and transformers for the tokenizer and config; transformers never needs torch for those). The three neural pieces — DeBERTa encoder (~97% of extraction time), boundary head, relation scorer — are ONNX graphs shipped in the package without their weights (`gliner_demo/onnx/graphs/<model>/`, ~2 MB). The weights are the checkpoint's own `model.safetensors`, downloaded from Hugging Face at the revision pinned in `meta.json` — the same file the torch backend downloads — and read with numpy (`safetensors_numpy.py`); every weight is a checkpoint tensor as-is or transposed, and its shape/dtype is checked at load. Graphs are regenerated by `python -m cognee.tasks.graph.gliner_demo.onnx.export` (needs the `gliner` extra, i.e. torch); only the default model ships graphs. gliner2's tokenization/batching/windowing/decoding is copied into `gliner_demo/onnx/_gliner2` by `vendor_gliner2.py`, byte-identical to upstream except import lines: `torch` resolves to `_tensor` (numpy implementations of the ~46 tensor ops that code uses); `transformers` is the real package, and the tokenizer loads through gliner2's own `load_extractor_tokenizer`, exactly as on the torch path. Never hand-edit `_gliner2`; re-run the vendor script (a test regenerates it and compares byte for byte; it is excluded from ruff/pre-commit). The export keeps shapes symbolic (two gliner2 helpers that pin the text length are rewritten export-only), exports the head for texts ≥32 words (the runtime pads shorter ones) and verifies every graph against torch. A model without shipped graphs, or graphs from another gliner2 version, is a hard error naming the export command; entity attributes/choice fields and records raise. Code: `cognee/tasks/graph/gliner_demo/onnx/`.
- **Demo**: `examples/guides/gliner_demo_llm_free_cognify.py`. Unit tests: `cognee/tests/unit/tasks/graph/test_gliner_demo_tasks.py`, `test_gliner_onnx_backend.py`, `test_gliner_onnx_tensor.py` (numpy ops vs torch), `test_gliner_onnx_vendored.py` (copy integrity); real-model parity (opt-in, `GLINER_ONNX_PARITY=1`): `test_gliner_onnx_parity.py`.

...
Runs under the per-dataset lock with the dataset-scoped database context (multi-tenant-safe). Falls back to the full delete + re-add + cognify flow when preconditions fail (first ingestion, non-text content, pre-v2 chunk ownership, or an unverified graph adapter — verified: Kuzu/Ladybug and Neo4j; Postgres and Neptune fall back); disable with `update(..., chunk_level_diff=False)` or the same query param on `PATCH /update`.

...
- **Scope / limitations**: only the 1-hop neighbourhood of the touched entities is compared; structural edges (`contains`, `is_part_of`, `made_from`, `exists_in`, `contradicts`) and edges with an unnamed endpoint are skipped.

### Temporal Memory (`temporal_cognify`)
`remember(..., temporal_cognify=True)` / `cognify(temporal_cognify=True)` layers a timeline on the standard graph. Inside `extract_graph_and_summarize` a third concurrent LLM lane extracts dated `Event` nodes (with `Timestamp` / `Interval` nodes; an interval's end is widened to the end of the unit the text stated) and each event is linked deterministically, via `involves` edges, to the entities of its chunk that it mentions. Summaries, ontology grounding, contradiction detection, provenance, `dry_run` (adds a `temporal_event_extraction` stage) and `chunk_attachment` all keep working. Cost: one extra LLM call per chunk. `TEMPORAL_LLM_EVENT_ENTITIES=true` adds an optional second LLM pass that names event roles.

Fact validity is one notion across the graph: `close_node` stamps `valid_to` on a node, and `resolve_temporal_contradictions` (opt-in via `functional_relationships`) tags the edge a newer assertion replaced with `superseded` **and** the same `valid_to`. Triplet retrieval projects both fields, ranks a stale fact below an equally relevant current one (`STALE_DISTANCE_FACTOR` in `cognee/modules/graph/utils/fact_validity.py`) and renders it with a `[valid until …; superseded …]` marker instead of dropping it. "Current" is judged as of now, or as of the window start for a temporal question, so "who was CEO in 2019" still sees a fact closed in 2020.

Query with `SearchType.TEMPORAL`: one LLM call parses the question into a time window (a year-only bound is widened to the whole year), the events anchored inside it are ranked by vector similarity and rendered chronologically. Windowless questions, empty windows, and backends without `collect_time_ids` / `collect_events` (Neptune, `postgres_demo`) fall back to triplet search.

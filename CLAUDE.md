...
- **LLM-free mode side effects** (when the env setting is `gliner`): the first-run environment check skips the LLM connection probe but still probes embeddings, and `recall()` with no `query_type` defaults to `CHUNKS` instead of routing to a completion type (explicit `query_type` is always honoured). `COGNEE_SKIP_CONNECTION_TEST` stays `false` by default.

...
- **ONNX Runtime backend** (`GLINER_BACKEND=onnx`, default `torch`): the three neural pieces gliner2 runs — DeBERTa encoder (~99% of inference time), boundary head, relation scorer — execute on ONNX Runtime from a one-time export (`python -m cognee.tasks.graph.gliner_demo.onnx.export`, needs the `gliner` extra; writes to `~/.cognee/models/gliner-onnx/<model>`, `GLINER_ONNX_DIR` moves it). Tokenization, batching, windowing and decoding stay gliner2's own Python, so torch is still imported at runtime; this backend proves the model runs on ONNX Runtime with matching output, it does not yet remove torch. The export keeps shapes symbolic (two gliner2 helpers that pin the text length are rewritten export-only), exports the head for texts ≥32 words (the runtime pads shorter ones) and verifies every graph against torch before keeping it. A missing export, an export of another model or of another gliner2 version is a hard error naming the export command. Entity attributes / choice fields (`score_explicit_spans`) are not exported and raise. Code: `cognee/tasks/graph/gliner_demo/onnx/`.
- **Demo**: `examples/guides/gliner_demo_llm_free_cognify.py`. Unit tests: `cognee/tests/unit/tasks/graph/test_gliner_demo_tasks.py`, `test_gliner_onnx_backend.py`; real-model parity (opt-in, `GLINER_ONNX_PARITY=1`): `test_gliner_onnx_parity.py`.

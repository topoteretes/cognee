...
Relational ingestion (`migrate_relational_database`, both schema-only and full, and the DLT schema graph) also aligns the technical schema with the ontology when one is configured: each table node whose name resolves to an ontology class (schema prefix stripped, plural tried as singular) gets a `realizes` edge to that class's `EntityType` node, and the class's own ontology subgraph is written alongside — the first version of the business ↔ technical mapping. Module: `cognee/modules/ontology/schema_alignment.py`.

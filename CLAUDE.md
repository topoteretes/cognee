...
- **Applies to `remember()` too** — and to session memory bridged back by `improve()` — since those build their graphs through `cognify()`. The exception is code: code files and repositories take cognify's CODE / CODE_REPO routes, which run enola only.
...
- **Search**: code is searchable through `SearchType.CODE` only (deterministic graph operations via `code_query`: `query_facts`, `explore`, `traverse`, `find_path`, `impact_analysis`, `insights`, `architecture`, `delta`). Completion/chunk search types (`GRAPH_COMPLETION`, `CHUNKS`, `RAG_COMPLETION`) do not cover code — the route produces no chunks and, by default, no embeddings. `cognify(index_vectors=True)` / `remember(index_vectors=True)` also embeds the code facts of the code files and repositories that run builds (an unchanged, already-built item is skipped by incremental loading).

...
- `e2e/` - Full-stack end-to-end suites run per backend in CI (e.g. `e2e/incremental_update/` runs on LadybugDB + LanceDB, Postgres graph + PGVector, and Neo4j + LanceDB; `e2e/keyless/` proves ingestion with no LLM key on real local models from core deps only, the GLiNER runtime installed at first use)
...
- **Install**: none needed. torch is not a cognee dependency (PyPI's Linux torch bundles several GB of CUDA libraries), so LLM-key users never download it. When `resolve_extractor()` resolves to `gliner_demo`, `cognee/tasks/graph/gliner_demo/install.py` installs what is missing, with progress logged (step, installer lines, elapsed time): torch only when no torch build (CPU or GPU) is importable, from `GLINER_TORCH_INDEX_URL` (default `https://download.pytorch.org/whl/cpu`, the only package taken from that index); then, when `gliner2` is missing, the `gliner` extra's requirements (read from cognee's metadata) from the default index with every installed distribution pinned — a package replaced under a live import would break the running process, so an install that needs to change one fails instead (core pins `huggingface-hub<1` and `tokenizers<=0.23.0`, the ranges transformers 4.x needs, for this). It uses pip, or `uv pip` in uv venvs, under a file lock in `sys.prefix`. The Docker image runs it at build time (`python -m cognee.tasks.graph.gliner_demo.install`, after the exact `uv sync`); `pip install "cognee[gliner]"` keeps torch under pip's control (CUDA on Linux, for GPU hosts); `GLINER_AUTO_INSTALL=false` raises `KeylessExtractorNotInstalledError` instead. A later exact `uv sync` removes the runtime-installed packages (they are not in the lockfile); the next GLiNER cognify installs them again. The model (`fastino/gliner2.5-base-v1`, ~800 MB) downloads on first use.

...
- **LLM-free mode side effects**: a pipeline with no LLM task (the gliner_demo extractor with contradiction detection off) skips the first-run LLM connection probe but still probes embeddings, per capability — an LLM-free run never marks the LLM check done for later LLM runs. Separately, with no usable LLM key `recall()` still auto-routes, but only to search types that run without an LLM: retrieval-only types run as they are, completion types whose single LLM call is the final answer (`HYBRID_COMPLETION`, `GRAPH_COMPLETION`, `RAG_COMPLETION`, `TRIPLET_COMPLETION`) run with `only_context=True` and return the assembled prompt, and types that call an LLM during retrieval resolve to the default, `HYBRID_COMPLETION` with `only_context=True`. The same `RoutingConstraints` (`cognee/api/v1/recall/query_router.py`) keep `CYPHER` and `NATURAL_LANGUAGE` out of routing when `ALLOW_CYPHER_QUERY=false`. Keyed on LLM availability, not on the extractor; an explicit `query_type` is always honoured as given. `COGNEE_SKIP_CONNECTION_TEST` stays `false` by default.
...
- **enola version**: installed as the `enola-cli` wheel through the `codegraph` extra (`pip install cognee[codegraph]`), which puts the binary in the environment's scripts directory; `ENOLA_PATH` always wins. Without the extra, the CODE route raises `EnolaNotInstalledError` with the install hint — nothing is downloaded at runtime. The version pin lives only in `pyproject.toml`. Cognee reads enola's documented snapshot contract (`facts.jsonl`, `insights.json`, `receipt.json`; `format_version` 1) and rejects a receipt with a format version it does not understand. Fact ids and resolved relation `target_id`s from the writer are used when present; explainer findings become `CodeInsight` nodes with `evidences` edges; the receipt's provenance/quality block is stamped on the `CodeRepository` node and reported by the `delta` operation. Bumping the pin means re-checking the known answers in `cognee/tests/test_code_graph_e2e.py`.

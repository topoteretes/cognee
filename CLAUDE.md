...
- **codegraph** - Empty, kept for backwards compatibility: the enola code-graph binary (`enola-cli`) is a base dependency
...
- **enola version**: `enola-cli` is a base dependency of cognee (platform wheels for macOS arm64/x86_64, Linux x86_64/aarch64 glibc, Windows amd64), which puts the binary in the environment's scripts directory; `ENOLA_PATH` always wins. A missing binary raises `EnolaNotInstalledError` — nothing is downloaded at runtime. The version pin lives only in `pyproject.toml`; the `codegraph` extra is empty and kept for backwards compatibility. Cognee reads enola's documented snapshot contract (`facts.jsonl`, `insights.json`, `receipt.json`; `format_version` 1) and rejects a receipt with a format version it does not understand. Fact ids and resolved relation `target_id`s from the writer are used when present; explainer findings become `CodeInsight` nodes with `evidences` edges; the receipt's provenance/quality block is stamped on the `CodeRepository` node and reported by the `delta` operation. Bumping the pin means re-checking the known answers in `cognee/tests/test_code_graph_e2e.py`.

...
`improve()` is an explicit orchestrator over an ordered registry of ten stages (`cognee/modules/improve/registry.py:DEFAULT_STAGES`): `feedback_weights`, `persist_session_qa`, `persist_agent_traces`, `extract_agent_context`, `distill_sessions`, `update_user_preferences`, `build_truth_subspace`, `triplet_enrichment`, `global_context_index`, `ontology_proposals`. The first seven need `session_ids`; the last three work on the graph alone. Order is load-bearing (4 feeds 5, 5 feeds 7, 7 runs before 8, 10 reads what the rest produced) and a test pins it. Stage 10 drafts governed-model proposals for a human to ratify (see "Working with Ontologies"); `IMPROVE_ONTOLOGY_PROPOSALS_ENABLED=false` skips it with `opt_in_disabled`.
...
The ontology is also used at read time (`ONTOLOGY_QUERY_GROUNDING`, default true, only active with `ONTOLOGY_FILE_PATH` set). `GraphCompletionRetriever` (and its COT / context-extension / summary / decomposition subclasses) and `HybridRetriever` resolve the query's words — longest n-grams first, stopwords dropped — against the ontology's classes and individuals with the same fuzzy matcher cognify uses, then (a) pin the matched nodes as exact-score seeds (`brute_force_triplet_search(pinned_node_ids=...)` / the hybrid entity lane), under the deterministic ids cognify wrote them with (`EntityType.id_for` / `Entity.id_for`), and (b) prepend an `## Ontology grounding` block to the context (`"credit exposure" refers to CreditExposure (class); is a RiskMetric`). No LLM or embedding call; the block only accompanies non-empty retrieved context, so "nothing found" stays detectable; batch queries are not grounded. Per-retriever override: `ontology_grounding=False` / `ontology_resolver=...`. Module: `cognee/modules/ontology/query_grounding.py`.

Relational ingestion (`migrate_relational_database`, both schema-only and full, and the DLT schema graph) also aligns the technical schema with the ontology when one is configured: each table node whose name resolves to an ontology class (schema prefix stripped, plural tried as singular) gets a `realizes` edge to that class's `EntityType` node, and the class's own ontology subgraph is written alongside — the business ↔ technical mapping. It goes one level down: the resolver also indexes `owl:ObjectProperty` / `owl:DatatypeProperty` (`lookup["properties"]`, with `domain` / `range` edges), and a column whose name resolves to a property (`email` → `hasEmail`; an abbreviated `cust_id` on a table realizing `Customer` tries `has_customer_id`) gets a `SchemaColumn` node — created only for such columns — with `table -has_column-> column -realizes-> OntologyProperty`; a property whose domain does not cover the table's class is skipped. Every `realizes` edge carries `authoritative` / `owner`, set from `ontology_config["authoritative_sources"]` (the same dict `cognify()` accepts, also reachable as `ONTOLOGY_AUTHORITATIVE_SOURCES`: a JSON object or `table[=owner],…`); the hybrid entity lane sorts authoritative edges first and labels them, so when several tables realize one concept the system of record is what the LLM sees. Module: `cognee/modules/ontology/schema_alignment.py`.

Cognify's own matching closes two gaps: a fuzzy class/individual match on a multi-word string is rejected unless every content token survives into the match (`enterprise customer acme` no longer collapses into `EnterpriseCustomer`), and an entity whose type found no class is grounded in the class its *name* ends with (`Credit exposure to Acme Corporation` typed `financial metric` becomes an `is_a CreditExposure`; the head noun must be the class, so `customer feedback form` is not a `Customer`). Shared string rules: `cognee/modules/ontology/term_matching.py`.

**Governed-model proposals** turn "LLM proposes, human ratifies" into one mechanism (`cognee/modules/proposals/`). The `ontology_proposals` improve stage drafts `OntologyProposal` nodes of three kinds — `mapping` (a schema table with no `realizes` edge: the class its name or `*_id` columns point at, else the LLM's pick from the table's description, columns and sample rows, with its confidence and `proposed_by="llm"`), `definition_conflict` (an unresolved `contradicts` edge from contradiction detection), `ontology_extension` (an ungrounded `EntityType` at least `IMPROVE_ONTOLOGY_PROPOSALS_MIN_OCCURRENCES` (3) entities are typed with). Nothing changes until someone ratifies: applying writes the `realizes` edge / the `resolution` onto the `contradicts` edge / `ontology_valid=True` on the `EntityType`, each stamped `ontology_valid=True`, `ratified_by`, `ratified_at`, `proposal_id`; rejecting only records it, and ids are deterministic so a decided finding is never re-drafted. Review over HTTP: `GET /api/v1/proposals/ontology?dataset_id=…[&kind=…&status=proposed|applied|rejected|all]`, `GET /api/v1/proposals/ontology/{proposal_id}`, `POST /api/v1/proposals/ontology/{proposal_id}/ratify` with `{"decision": "apply"|"reject", "resolution": "…"}` (write permission); SDK: `cognee.modules.proposals.list_ontology_proposals` / `ratify_ontology_proposal`.

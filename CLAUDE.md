...
`update(data, dataset_id, data_id=None)` replaces an existing document in place. It diffs the new content against the stored processed text (`Data.raw_data_location`) and replaces only the chunks the edits touched: a paragraph-anchored multi-region diff (near-linear for any change shape, including repeated-line-heavy content) finds every disjoint changed span (so edits at the top, middle, and end of a document are three small regions, not one giant one), each span is expanded to chunk boundaries and re-chunked with the standard TextChunker (same boundary semantics as pipeline chunks, cut against the token budget recorded on the chunks it replaces — every chunk stores `max_chunk_tokens`, so documents stay self-consistent across config changes; legacy chunks without the field fall back to the current config; only a region's last chunk may be under-filled), replaced chunks (plus their summaries, chunk-orphaned entities, and triplet embeddings) are deleted, and only the new chunks run through LLM extraction. Chunks between regions are kept without being re-chunked, so their boundaries and ids cannot drift. Unaffected chunks keep their node ids, entities, summaries, and embeddings; surviving chunks are renumbered so `chunk_index` stays contiguous. Chunk identity is content-derived (`uuid5(doc : sha256(text) : occurrence)`, see `cognee/modules/chunking/chunk_id.py`), so unchanged content keeps its identity across edits. Both incremental and full fallback paths preserve `data_id`.

Runs under the per-dataset lock with the dataset-scoped database context (multi-tenant-safe). Falls back to a full rebuild when preconditions fail (first ingestion, non-text content, pre-v2 chunk ownership, an unverified graph adapter — verified: Kuzu/Ladybug, Neo4j and the Postgres demo adapter; Neptune falls back — or a document on the code / DLT cognify route, which keeps no chunks); disable with `update(..., chunk_level_diff=False)` or the same query param on `PATCH /update`. The rebuild never deletes the row: it drops the document's memory (`forget(memory_only=True)`), lets a pinned re-add refresh the row in place (content, name, metadata), and re-cognifies at the chunk budget the old chunks recorded. A re-add that fails leaves a document with no graph for the next `cognify()`, never a missing document.

**Which document.** `data_id` names it (exact, or a recorded pre-fork `legacy_id`; unknown → `UpdateTargetNotFoundError`, 404 — `update()` never creates). Without one, the document is inferred from the input's origin, the same lookup `add()` uses (`cognee/modules/ingestion/identify_by_origin.py`): a local file by its path (`Data.original_data_location`), an upload by its filename. Raw text (stored under a content-hash name), a renamed or moved file, an origin no document came from, or one shared by several documents raise `UpdateTargetNotInferredError` (422) before anything is written; the message says how to list the dataset's documents and pass `data_id`. The filename of a replacement is otherwise irrelevant: with `data_id`, a renamed file updates content and name in place.

**Batches.** A list of several inputs, or a directory, updates one document per file and returns `UpdateBatchResult` (a one-item list is that one document, as before): `status` (`completed` / `partial` / `failed`), `total`, `updated`, `unchanged`, `failed`, and `results`, one `UpdateResult` per input in input order. Every target is resolved before the first document is touched; a document whose update raises afterwards is recorded as `failed` with its error and the batch carries on, so the caller retries it by `data_id`. One `data_id` goes with one input; per-document ids travel as `DataItem(data=..., data_id=...)`. Over HTTP, several `data` files are a batch, each matched by filename.

**Result.** One dict on every path, `UpdateResult` (`cognee/api/v1/update/result.py`), a superset of the v1.5.4 summary: `status` (`incremental` / `unchanged` / `full_rebuild` / `failed`), the chunk counters plus `total_chunks` (all `null` on a rebuild), `data_id`, `dataset_id`, `duration_seconds`, `pipeline_run_id`, `fallback` (`reason` from the nine-value `RefusalReason` enum and a `detail` sentence, set on every rebuild) and `error` (`error_class`, `message` on `failed`). The route keeps the 500 status for `failed` but sends this body; `serve()` returns the same dict.

**add() and updates.** `add()` creates documents and leaves existing ones alone: a file the dataset already holds by origin with different content raises `DocumentUpdateRequiredError` (409 on `POST /add`) naming the `data_id` and pointing at `update()`; the check runs once over the whole request, so a batch with one changed file is refused whole and nothing is written. Identical content re-added is a no-op.

Key files:
- `cognee/api/v1/update/update.py` / `cognee/api/v1/update/incremental.py` / `cognee/api/v1/update/result.py`
- `cognee/modules/ingestion/identify_by_origin.py` (origin lookup shared by add() and update())
- `cognee/tasks/ingestion/refuse_changed_existing_documents.py` (the add() refusal)

            ...
            try:
                ...
                for key, display_name in standard_metrics.items():
                    ...
                    if key in data and isinstance(data[key], dict) and "mean" in data[key]:
                        ...
                        print(f"Found DeepEval metrics in {main_metrics_file}: {key}={data[key]['mean']:.4f}")
            except Exception as e:
            ...
            try:
                ...
                for key, display_name in standard_metrics.items():
                    ...
                    if key in data and isinstance(data[key], dict) and "mean" in data[key]:
                        ...
                        print(f"Found DeepEval metrics in {metrics_file}: {key}={data[key]['mean']:.4f}")
            except Exception as e:
                        ...
                        if isinstance(data, dict):
                            ...
                            for key, display_name in standard_metrics.items():
                                ...
                                if key in data and isinstance(data[key], dict) and "mean" in data[key]:
                                    ...
                                    print(f"Found DeepEval metrics in {file}: {key}={data[key]['mean']:.4f}")
                except Exception as e:
                    print(f"Error loading {file}: {e}")
...
def plot_metrics(all_systems_metrics):
    ...
    colors = [brand_colors['data_flux_green'], brand_colors['data_dream_violet'], 
              brand_colors['secondary_purple'], brand_colors['dark_grey']]
    ...
    for i, metric in enumerate(metrics):
        ...
        ax.bar(x + i * width - (len(metrics)-1) * width/2, 
               means,
               width,
               label=metric,
    ...
    try:
        ...
        if logo_path.exists():
            ...
            fig_with_logo = plt.figure(figsize=(height, width), facecolor=brand_colors['data_cloud_grey'])

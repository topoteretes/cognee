...
async def run_question_answering(params: dict, questions_path: str, answers_path: str) -> None:
    if params.get("answering_questions"):
        ...
        try:
            ...
            raise FileNotFoundError(f"Could not find the file: {questions_path}")
        except json.JSONDecodeError as e:
            raise ValueError(f"Error decoding JSON from {questions_path}: {e}")

        logging.info(f"Loaded {len(questions)} questions from {questions_path}")

...
export default function CustomAppShellNavbar() {
  ...
  // Integrations (Slack) is a multi-tenant, management-API feature with no
  // equivalent in single-tenant OSS mode — the route itself is excluded from
  // the public sync (CLO-686), so the link must disappear here too or it 404s.
  const navSections = isCloudEnvironment()
    ? NAV_SECTIONS
    : NAV_SECTIONS.map((section) =>
        section.label === "CONNECT"
          ? { ...section, items: section.items.filter((item) => item.link !== "/integrations") }
          : section,
      );

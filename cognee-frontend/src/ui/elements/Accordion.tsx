  ...
  return (
    <div className={classNames("flex flex-col", className)}>
      <div className="flex flex-row justify-between items-center">
        <button className={classNames("flex flex-row items-center pr-2 cursor-pointer", switchCaretPosition ? "gap-1.5" : "gap-4")} onClick={isOpen ? closeAccordion : openAccordion}>
          {switchCaretPosition ? (
            <>
              <CaretIcon className={classNames("transition-transform duration-300", isOpen ? "rotate-360" : "rotate-270")} />
              {title}
            </>
          ) : (
            <>
              {title}
              <CaretIcon className={classNames("transition-transform duration-300", isOpen ? "rotate-0" : "rotate-180")} />
            </>
          )}
        </button>
        {tools}

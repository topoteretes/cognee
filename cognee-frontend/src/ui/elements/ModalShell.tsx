...
export default function ModalShell({
  ...
  width?: number;
  /**
   * Accessible name for the dialog. Callers render their own heading rather
   * than Mantine's Modal.Title, and Modal.Content hard-overrides
   * aria-labelledby with its own (undefined when no Modal.Title is mounted),
   * so the name has to come through aria-label, which it does forward.
   */
  label?: string;
  children: ReactNode;
}): ReactElement {

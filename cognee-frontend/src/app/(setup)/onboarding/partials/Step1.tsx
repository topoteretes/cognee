...
import { StepDots, OnboardingFooter } from "./Shared";
...
export function Step1({ onNext, onBack, files, setFiles }: {
  onNext: () => void;
  onBack: () => void;
  files: File[];
  ...
  const handlePasteSubmit = () => {
    ...
    track({ pageName: "Onboarding", eventName: "onboarding_text_pasted", additionalProperties: { text_length: String(pasteText.length), step: "1" } });
  ...
  return (
    <div style={{
      ...
      <div style={{ alignItems: "center", display: "flex", flexDirection: "column", paddingBottom: 40, paddingLeft: "clamp(16px, 5vw, 80px)", paddingRight: "clamp(16px, 5vw, 80px)", paddingTop: 48, width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ maxWidth: 880, width: "100%", background: "#2a2a2e", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 0, boxShadow: "0 20px 60px rgba(0,0,0,0.5)", display: "flex", flexDirection: "column", gap: 20, padding: "48px 64px" }}>
          ...
          <div
            ...
            style={{ alignItems: "center", background: isDragging ? "rgba(188,155,255,0.20)" : "rgba(255,255,255,0.04)", border: `2px dashed ${isDragging ? "#BC9BFF" : "rgba(255,255,255,0.18)"}`, borderRadius: 0, cursor: "pointer", display: "flex", flexDirection: "column", flexShrink: 0, gap: 8, height: 200, justifyContent: "center", paddingBlock: 40, paddingInline: 20, transition: "background 200ms, border-color 200ms" }}
          >
            <div style={{ alignItems: "center", background: "rgba(255,255,255,0.1)", borderRadius: 0, display: "flex", flexShrink: 0, height: 40, justifyContent: "center", width: 40 }}>
          ...
          {files.length > 0 && (
            <div className="flex flex-col gap-2">
              {files.map((f, i) => (
                <div key={`f-${i}`} className="flex items-center justify-between" style={{ background: "rgba(255,255,255,0.07)", borderRadius: 0, padding: "8px 12px" }}>
          ...
          {!showPaste ? (
            <div onClick={() => setShowPaste(true)} style={{ alignItems: "center", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 0, cursor: "pointer", display: "flex", gap: 8, paddingBlock: 12, paddingInline: 16 }}>
            ...
            <div className="flex flex-col gap-2">
              <textarea
                ...
                style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 0, color: "#EDECEA", fontSize: 13, minHeight: 80, padding: 12, resize: "vertical", outline: "none" }}
              />
              <div className="flex gap-2">
                <button onClick={handlePasteSubmit} className="cursor-pointer" style={{ background: "#BC9BFF", border: "none", borderRadius: 0, color: "#1e1e1c", fontSize: 13, padding: "6px 16px" }}>Add text</button>
                <button onClick={() => { setShowPaste(false); setPasteText(""); }} className="cursor-pointer" style={{ background: "none", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 0, color: "rgba(237,236,234,0.6)", fontSize: 13, padding: "6px 16px" }}>Cancel</button>
              </div>
            </div>
          )}
          ...
          <OnboardingFooter
            onBack={onBack}
            onContinue={() => {
              track({ pageName: "Onboarding", eventName: "onboarding_step_completed", additionalProperties: { step: "1", file_count: String(files.length) } });
              onNext();
            }}

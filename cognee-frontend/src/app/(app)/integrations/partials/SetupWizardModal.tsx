...
export default function SetupWizardModal({ cfg, steps, currentStep, onClose, onStepSelect }: SetupWizardModalProps): ReactElement {
  ...
  return (
    <div role="dialog" aria-modal="true" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div className="aci-popup" onClick={(e) => e.stopPropagation()} style={{ background: "rgba(15,15,15,0.92)", backdropFilter: "blur(16px)", borderRadius: 0, width: 520, maxWidth: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.1)", overflow: "hidden", animation: "aci-popup 200ms cubic-bezier(0.22,1,0.36,1) forwards" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          {/* Shell-based connectors render different commands per OS; MCP ones
              ignore it, so the toggle only appears where it changes something. */}
          {cfg.osAware && <OsToggle />}
          <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(237,236,234,0.65)", cursor: "pointer", padding: 4, borderRadius: 0, lineHeight: 1 }}>
          ...
          return (
            <div key={i} className="aci-step-row" data-active={isStepActive ? "true" : undefined} onClick={() => onStepSelect(i)} style={{ borderBottom: i < steps.length - 1 ? "1px solid rgba(255,255,255,0.07)" : "none", cursor: isStepActive ? "default" : "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: isStepActive ? "14px 20px 0" : "14px 20px" }}>
                <div style={{ width: 24, height: 24, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: isDone ? "#DCFCE7" : isStepActive ? "var(--color-cognee-lavender)" : "#F4F4F5", transition: "background 200ms" }}>
                  {isDone
                ...
                {isDone && <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase", background: "rgba(34,197,94,0.12)", color: "#22C55E", borderRadius: 100, padding: "2px 8px", flexShrink: 0 }}>Done</span>}
              </div>
              <div style={{ display: "grid", gridTemplateRows: isStepActive ? "1fr" : "0fr", opacity: isStepActive ? 1 : 0, transition: "grid-template-rows 260ms ease, opacity 200ms ease" }}>
                <div style={{ overflow: "hidden" }}>
                  <div onClick={(e) => e.stopPropagation()} style={{ padding: "10px 20px 18px 56px" }}>
                    ...
                    {step.lines && <TerminalBlock lines={step.lines} loading={step.loading} placeholder="Preparing your credentials…" copyLabel={step.copyLabel} />}
                    ...
                    {i < steps.length - 1
                      ...
                      : <button onClick={(e) => { e.stopPropagation(); onClose(); router.push("/sessions"); }} style={{ marginTop: 12, display: "inline-flex", alignItems: "center", gap: 5, background: "none", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 0, padding: "7px 14px", fontSize: 13, fontWeight: 500, color: "rgba(237,236,234,0.7)", fontFamily: "inherit", cursor: "pointer" }}>Go to Sessions →</button>}
                  </div>
                </div>
              </div>
            </div>

...
export default function AddDataToCognee({ datasets, refreshDatasets, instance }: AddDataToCogneeProps) {
  const useCloud = isCloudEnvironment();
  ...
  }, [filesForUpload, instance, refreshDatasets]);
  ...
  return (
    <>
      <GhostButton onClick={openAddDataModal} className="mb-5 py-1.5 !px-2 text-sm w-full items-center justify-start">
        {isProcessingDataWithCognee ? <LoadingIndicator /> : <PlusIcon />}
        Add data to cognee
      </GhostButton>
          ...
          <form onSubmit={submitDataToCognee}>
            <div className="max-w-md flex flex-col gap-4">
              ...
              <NeutralButton className="w-full relative justify-start pl-4">
                <input disabled={isProcessingDataWithCognee} onChange={addFiles} required name="files" tabIndex={-1} type="file" multiple className="absolute w-full h-full cursor-pointer opacity-0" />

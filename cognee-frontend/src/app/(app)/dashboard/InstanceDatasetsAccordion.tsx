...
import { setApiKey } from "@/modules/instances/localFetch";

interface InstanceDatasetsAccordionProps extends Omit<DatasetsAccordionProps, "title" | "instance"> {
  searchValue: string;
  instances: {
    localCognee: CogneeInstance;
    cloudCognee: CogneeInstance;
  }
}

export default function InstanceDatasetsAccordion({ searchValue, onDatasetsChange, instances }: InstanceDatasetsAccordionProps) {
  const {
  ...
  const checkConnectionToCloudCognee = useCallback((apiKey?: string) => {
      ...
      return instances.cloudCognee.fetch("/health")
        .then(setCloudCogneeConnected)
    }, [instances.cloudCognee, setCloudCogneeConnected]);

  useEffect(() => {
    const checkConnectionToLocalCognee = () => {
      instances.localCognee.fetch("/health")
        .then(setLocalCogneeConnected)
    };

    checkConnectionToLocalCognee();
    checkConnectionToCloudCognee();
  }, [checkConnectionToCloudCognee, instances.localCognee, setCloudCogneeConnected, setLocalCogneeConnected]);
  ...
  return (
    <div className={classNames("flex flex-col", {
      ...
      {isLocalCogneeConnected ? (
        <DatasetsAccordion
          title={(
            <div className="flex flex-row items-center justify-between">
              <div className="flex flex-row items-center gap-2">
                <LocalCogneeIcon className="text-cognee-purple" />
                <span className="text-xs">local cognee</span>
              </div>
            </div>
          )}
          searchValue={searchValue}
          tools={isLocalCogneeConnected ? <span className="text-xs text-cognee-purple">Connected</span> : <span className="text-xs text-gray-400">Not connected</span>}
          switchCaretPosition={true}
          className="pt-3 pb-1.5"
          contentClassName="pl-4"
          onDatasetsChange={!isCloudEnv ? onDatasetsChange : () => {}}
          instance={instances.localCognee}
        />
      ) : (
        <div className="w-full flex flex-row items-center justify-between py-1.5 cursor-pointer pt-3">
      ...
      {isCloudCogneeConnected ? (
        <DatasetsAccordion
          ...
          searchValue={searchValue}
          tools={<span className="text-xs text-cognee-purple">Connected</span>}
          switchCaretPosition={true}
          className="pt-3 pb-1.5"
          contentClassName="pl-4"
          onDatasetsChange={isCloudEnv ? onDatasetsChange : () => {}}
          instance={instances.cloudCognee}
        />
      ) : (
      ...
      <Modal isOpen={isCloudConnectedModalOpen}>
        <div className="w-full max-w-2xl">
          ...
          <div className="mt-8 mb-6">Please provide your API key. You can find it on <a className="!text-cognee-purple" href="https://platform.cognee.ai">our platform.</a></div>
